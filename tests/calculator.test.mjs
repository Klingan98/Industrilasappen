...
test("utan kontinuitetsprioritering kan kvällsarbetet delas mellan flera order", () => {
  const schedule = recommendAutomaticEveningSchedule({
    orders: [
  ...
  assert.equal(schedule.strategy.riskWithRecommendation, 0);
  assert.equal(schedule.strategy.eveningNeeded, true);
  assert.equal(schedule.strategy.eveningOrderIds.length, 2);
});
...
test("två identiska pressar används parallellt av två order när två montörer finns", () => {
  const schedule = recommendProductionSchedule({
    orders: [
      { id: "A", dueAt: new Date(2026, 8, 7, 16, 0).toISOString(), moments: [{ id: "A1", name: "Pressning", laborSeconds: 7200, maxStaff: 2, resourceName: "Press", resourceCapacity: 2 }] },
      { id: "B", dueAt: new Date(2026, 8, 7, 16, 0).toISOString(), moments: [{ id: "B1", name: "Pressning", laborSeconds: 7200, maxStaff: 2, resourceName: "Press", resourceCapacity: 2 }] },
    ],
    start: new Date(2026, 8, 7, 7, 0),
    settings: DEFAULT_SETTINGS,
    dayStaff: 2,
  ...
  const firstSegments = schedule.segments.filter((segment) => new Date(segment.start).getHours() === 7);
  assert.equal(firstSegments.length, 2);
  assert.deepEqual(firstSegments.map((segment) => segment.staff).sort(), [1, 1]);
  assert.ok(firstSegments.every((segment) => segment.resourceCapacity === 2));
  assert.ok(schedule.orders.every((order) => new Date(order.finishAt).getHours() === 9));
});

test("en ensam order kan använda båda pressarna när två montörer finns", () => {
  const schedule = recommendProductionSchedule({
    orders: [
      { id: "A", dueAt: new Date(2026, 8, 7, 16, 0).toISOString(), moments: [{ id: "A1", name: "Pressning", laborSeconds: 7200, maxStaff: 2, resourceName: "Press", resourceCapacity: 2 }] },
    ],
  ...
  assert.equal(schedule.segments[0].staff, 2);
  assert.equal(new Date(schedule.orders[0].finishAt).getHours(), 8);
});

test("kvällsskift fördelas till ordern som minskar total deadlineförsening mest", () => {
  const schedule = recommendAutomaticEveningSchedule({
    orders: [
      { id: "Stor", laborSeconds: 12 * 60 * 60, dueAt: new Date(2026, 8, 7, 23, 0).toISOString() },
      { id: "Liten", laborSeconds: 4 * 60 * 60, dueAt: new Date(2026, 8, 7, 23, 0).toISOString() },
    ],
    start: new Date(2026, 8, 7, 7, 0),
    settings: DEFAULT_SETTINGS,
    dayStaff: 1,
  ...
  assert.deepEqual(schedule.strategy.eveningOrderIds, ["Stor"]);
  assert.ok(schedule.strategy.totalLatenessWithRecommendationMilliseconds < schedule.strategy.totalLatenessWithoutEveningMilliseconds);
});


test("omställningstid räknas när en resursplats byter till en annan order", () => {
  const schedule = recommendProductionSchedule({
    orders: [
      { id: "A", dueAt: new Date(2026, 8, 7, 12, 0).toISOString(), moments: [{ id: "A1", name: "Pressning", laborSeconds: 3600, maxStaff: 1, resourceName: "Press", resourceCapacity: 1 }] },
      { id: "B", dueAt: new Date(2026, 8, 7, 16, 0).toISOString(), moments: [{ id: "B1", name: "Pressning", laborSeconds: 3600, maxStaff: 1, resourceName: "Press", resourceCapacity: 1 }] },
    ],
    start: new Date(2026, 8, 7, 7, 0),
    settings: DEFAULT_SETTINGS,
    dayStaff: 1,
  ...
  assert.equal(schedule.setupLaborSeconds, 15 * 60);
  assert.equal(schedule.changeoverCount, 1);
  const b = schedule.orders.find((order) => order.id === "B");
  assert.equal(new Date(b.finishAt).getHours(), 9);
  assert.equal(new Date(b.finishAt).getMinutes(), 15);
});

test("planerad rast skapar ingen ny omställning när samma order fortsätter", () => {
  const schedule = recommendProductionSchedule({
    orders: [
      { id: "A", dueAt: new Date(2026, 8, 7, 16, 0).toISOString(), moments: [{ id: "A1", name: "Pressning", laborSeconds: 2 * 3600, maxStaff: 1, resourceName: "Press", resourceCapacity: 1 }] },
    ],
    start: new Date(2026, 8, 7, 8, 30),
    settings: DEFAULT_SETTINGS,
    dayStaff: 1,
    eveningStaff: 0,
    setupMinutes: 15,
  });
  assert.equal(schedule.setupLaborSeconds, 0);
  assert.equal(schedule.changeoverCount, 0);
  assert.equal(new Date(schedule.orders[0].finishAt).getHours(), 10);
  assert.equal(new Date(schedule.orders[0].finishAt).getMinutes(), 45);
});

test("kontinuitetsprioritering kan låta båda pågående order fortsätta på kvällen för att slippa maskinbyte", () => {
  const orders = [
    { id: "A", dueAt: new Date(2026, 8, 8, 10, 0).toISOString(), moments: [{ id: "A1", name: "Pressning", laborSeconds: 9 * 3600, maxStaff: 2, resourceName: "Press", resourceCapacity: 2 }] },
    { id: "B", dueAt: new Date(2026, 8, 7, 18, 0).toISOString(), moments: [{ id: "B1", name: "Pressning", laborSeconds: 9 * 3600, maxStaff: 2, resourceName: "Press", resourceCapacity: 2 }] },
  ];
  const withoutContinuity = recommendAutomaticEveningSchedule({
    orders,
    start: new Date(2026, 8, 7, 7, 0),
    settings: DEFAULT_SETTINGS,
    dayStaff: 2,
  ...
  const withContinuity = recommendAutomaticEveningSchedule({
    orders,
    start: new Date(2026, 8, 7, 7, 0),
    settings: DEFAULT_SETTINGS,
    dayStaff: 2,
  ...
  assert.deepEqual(withoutContinuity.strategy.eveningOrderIds, ["B"]);
  assert.equal(withContinuity.strategy.eveningOrderIds.length, 2);
  assert.equal(withContinuity.setupLaborSeconds, 0);
  assert.ok(withoutContinuity.setupLaborSeconds > withContinuity.setupLaborSeconds);
});
