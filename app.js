...
function plannerCapacitySummary(order) {
  ...
  const resources = [...new Set(limited.map((moment) => String(moment.resourceName || "").trim()).filter(Boolean))];
  const labels = [];
  if (limited.length) labels.push(`${limited.length} ${limited.length === 1 ? "begränsat moment" : "begränsade moment"}`);
...
function renderPlannerSchedule(schedule) {
  ...
  dom.plannerScheduleList.innerHTML = groups.map((segment, index) => {
    ...
    const resource = segment.resourceName ? ` · ${segment.resourceName}` : "";
    return `
...
function renderPlanner() {
  ...
  if (note) {
    ...
    note.textContent = `Appen fördelar bemanningen mellan körbara delmoment och respekterar max montörer samt delade maskiner/fixturer. Den provar först bara dagskift och använder kväll först när det förbättrar deadlineutfallet. Dagskift ${state.settings.dayStart}–${state.settings.dayEnd}, fredag till ${state.settings.fridayEnd}${state.settings.fridayNoLunch === false ? "" : " utan lunchrast"}.${resourceText}`;
  }
}
