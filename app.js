...
function loadPlannerConfig() {
  try {
    ...
    return {
      ...
      prioritizeContinuity: saved.prioritizeContinuity !== false,
      setupMinutes: Math.max(0, Math.min(240, Number(saved.setupMinutes ?? 15) || 0)),
      restartMinutes: Math.max(0, Math.min(240, Number(saved.restartMinutes ?? 10) || 0)),
      minRunMinutes: Math.max(0, Math.min(480, Number(saved.minRunMinutes ?? 120) || 0)),
    };
  } catch {
    return {
...
function updatePlannerPresetSummary() {
  ...
  const limited = preset.moments.filter((moment) => moment.maxStaff || moment.resourceName).length;
  const capacityText = limited ? ` · ${limited} ${limited === 1 ? "kapacitetsstyrt moment" : "kapacitetsstyrda moment"}` : "";
  summary.textContent = `${preset.moments.length} delmoment · ${formatPieceTime(seconds)} original totalt · ${formatNumber(preset.bufferPercent)} % indirekt tid${capacityText}`;
}
...
function plannerConfigFromInputs() {
  ...
  return {
    ...
    prioritizeContinuity: $("#plannerPrioritizeContinuity").checked,
    setupMinutes: Math.max(0, Math.min(240, Number($("#plannerSetupMinutes").value) || 0)),
    restartMinutes: Math.max(0, Math.min(240, Number($("#plannerRestartMinutes").value) || 0)),
    minRunMinutes: Math.max(0, Math.min(480, Number($("#plannerMinRunMinutes").value) || 0)),
  };
}
...
function syncPlannerConfigInputs() {
  ...
  $("#plannerPrioritizeContinuity").checked = state.plannerConfig.prioritizeContinuity !== false;
  $("#plannerSetupMinutes").value = String(state.plannerConfig.setupMinutes ?? 15);
  $("#plannerRestartMinutes").value = String(state.plannerConfig.restartMinutes ?? 10);
  $("#plannerMinRunMinutes").value = String(state.plannerConfig.minRunMinutes ?? 120);
}
...
function buildPlannerRecommendation() {
  ...
  return recommendAutomaticEveningSchedule({
    ...
    prioritizeContinuity: state.plannerConfig.prioritizeContinuity !== false,
    setupMinutes: state.plannerConfig.setupMinutes ?? 15,
    restartMinutes: state.plannerConfig.restartMinutes ?? 10,
    minRunMinutes: state.plannerConfig.minRunMinutes ?? 120,
  });
}
        ...
        <article class="planned-order-card" data-planned-order="${escapeHtml(order.id)}">
          <div class="planned-order-main">
            ...
            <div class="planned-order-meta">
              ...
              <span><b>Planerad persontid</b> ${formatDuration(labor + Math.max(0, Number(result?.setupLaborSeconds) || 0), true)}</span>
              ${result?.setupLaborSeconds ? `<span><b>Omställning/återstart</b> ${formatDuration(result.setupLaborSeconds, true)}</span>` : ""}
              <span><b>Momentkapacitet</b> ${escapeHtml(plannerCapacitySummary(order))}</span>
  ...
  dom.plannerScheduleList.innerHTML = groups.map((group, index) => {
    ...
    const jobs = group.items.map((segment) => {
      ...
      const resource = segment.resourceName
        ...
        : "";
      const setup = Math.max(0, Number(segment.setupLaborSeconds) || 0);
      return `
        <div class="schedule-job">
          ...
          <div class="schedule-job-meta"><span>${segment.staff} ${segment.staff === 1 ? "montör" : "montörer"}</span>${resource}${setup ? `<span class="schedule-setup-badge">Omställning ${escapeHtml(formatDuration(setup, true))}</span>` : ""}</div>
        </div>`;
    }).join("");
...
function renderPlannerStrategyBanner(schedule) {
  ...
  const setupTime = Math.max(0, Number(schedule?.setupLaborSeconds) || 0);
  const setupText = setupTime ? ` Omställning/återstart i planen: ${formatDuration(setupTime, true)}.` : "";
  banner.className = `planner-strategy-banner ${risk ? "still-risk" : "needs-evening"}`;
  banner.innerHTML = risk
    ? `<span class="strategy-icon">!</span><div><strong>Kvällsskift rekommenderas, men kapaciteten räcker ändå inte helt</strong><span>Planen använder ${escapeHtml(shiftText)} (${escapeHtml(eveningTime)}). ${risk} ${risk === 1 ? "order riskerar" : "ordrar riskerar"} fortfarande att bli sen.${escapeHtml(setupText)} Prova mer bemanning eller tidigare planstart.</span></div>`
    : `<span class="strategy-icon">↗</span><div><strong>Kvällsskift rekommenderas</strong><span>Utan kväll skulle ${without} ${without === 1 ? "order riskera" : "ordrar riskera"} deadline. Bästa planen använder ${escapeHtml(shiftText)}, totalt ${escapeHtml(eveningTime)} kvällstid.${escapeHtml(setupText)}</span></div>`;
}

function renderPlannerSummary(schedule) {
  const baseLabor = state.plannedOrders.reduce((sum, order) => sum + plannerLaborSeconds(order), 0);
  const totalLabor = baseLabor + Math.max(0, Number(schedule?.setupLaborSeconds) || 0);
...
function renderPlanner() {
  ...
  if (note) {
    const resources = (schedule.resources || []).map((resource) => `${resource.name} ×${resource.capacity}`).join(", ");
    const resourceText = resources ? ` Delade resurser i aktuell plan: ${resources}.` : "";
    const maxResourceCapacity = Math.max(0, ...(schedule.resources || []).map((resource) => Number(resource.capacity) || 0));
    const staffingNote = maxResourceCapacity > state.plannerConfig.dayStaff
      ? ` Observera: största resurskapaciteten är ${maxResourceCapacity}, men dagskiftet har ${state.plannerConfig.dayStaff} ${state.plannerConfig.dayStaff === 1 ? "montör" : "montörer"}; fler parallella maskinplatser kan därför inte utnyttjas fullt på dagen.`
      : "";
    const continuityText = state.plannerConfig.prioritizeContinuity !== false
      ? ` Kontinuitet prioriteras: orderbyte kostar ${formatNumber(state.plannerConfig.setupMinutes ?? 15)} min, återstart ${formatNumber(state.plannerConfig.restartMinutes ?? 10)} min och frivilliga körblock försöker bli minst ${formatNumber(state.plannerConfig.minRunMinutes ?? 120)} min.`
      : " Kontinuitetsprioritering är avstängd; angiven omställningstid räknas ändå som kapacitetsförlust.";
    note.textContent = `Appen fördelar bemanningen parallellt mellan körbara delmoment, respekterar max montörer och delade maskiner/fixturer och prioriterar upplägget som minimerar den sammanlagda deadlineförseningen. Kväll används bara när det förbättrar helheten.${continuityText} Dagskift ${state.settings.dayStart}–${state.settings.dayEnd}, fredag till ${state.settings.fridayEnd}${state.settings.fridayNoLunch === false ? "" : " utan lunchrast"}.${resourceText}${staffingNote}`;
  }
}
  ...
  try {
    ...
    if (payload.plannerConfig) {
      ...
      state.plannerConfig = {
        ...
        prioritizeContinuity: payload.plannerConfig.prioritizeContinuity !== false,
        setupMinutes: Math.max(0, Math.min(240, Number(payload.plannerConfig.setupMinutes ?? 15) || 0)),
        restartMinutes: Math.max(0, Math.min(240, Number(payload.plannerConfig.restartMinutes ?? 10) || 0)),
        minRunMinutes: Math.max(0, Math.min(480, Number(payload.plannerConfig.minRunMinutes ?? 120) || 0)),
      };
      savePlannerConfig();
    }
    saveSettings();
...
function attachEvents() {
  ...
  ["#plannerStartAt", "#plannerDayStaff", "#plannerEveningStaff", "#plannerPrioritizeContinuity", "#plannerSetupMinutes", "#plannerRestartMinutes", "#plannerMinRunMinutes"].forEach((selector) => {
