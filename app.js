...
function groupedPlannerSegments(schedule) {
  ...
  for (const segment of schedule.segments) {
    ...
    if (previous && previous.orderId === segment.orderId && previous.momentId === segment.momentId && previous.shift === segment.shift && previous.dayKey === dayKey && previous.staff === segment.staff) {
...
function renderPlannerSchedule(schedule) {
  ...
  dom.plannerScheduleList.innerHTML = groups.map((segment, index) => {
    ...
    const resource = segment.resourceName
      ? ` · ${segment.resourceName}${resourceCapacity ? ` · ${segment.staff}/${resourceCapacity} resursplatser` : ""}`
      : "";
    return `
      <div class="schedule-row ${segment.shift === "evening" ? "is-evening" : ""}">
        ...
        <div class="schedule-order"><strong>${escapeHtml(order?.name || "Order")} · ${escapeHtml(moment)}</strong><span>${shift} · ${segment.staff} ${segment.staff === 1 ? "montör" : "montörer"}${escapeHtml(resource)}</span></div>
      </div>`;
  }).join("");
}
