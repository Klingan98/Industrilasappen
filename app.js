import {
  addWorkingSeconds,
  calculateMultiMomentPlan,
  calculateOrderPlan,
  cartonBreakdown,
  cloneDefaultSettings,
  plannedRemainingSeconds,
  preCompletedQuantity,
  productiveSecondsBetween,
  registeredProductionQuantity,
  settingsAreValid,
  totalCompletedQuantity,
  weightedMomentProgress,
  recommendAutomaticEveningSchedule,
} from "./calculator.js?v=11";
import {
  ORDER_TYPES, WORKPLACES, normalizePlanningOrder, normalizePlannerConfig,
  validatePlanningOrder, buildWorkshopPlan, operationLabel, momentLaborSeconds,
  orderLaborSeconds, isPlanningComplete, rotaryExample,
} from "./planning.js?v=11";

const SETTINGS_KEY = "industrilasappen.settings.v1";
const ORDER_KEY = "industrilasappen.active-order.v1";
const PRESETS_KEY = "industrilasappen.presets.v1";
const PLANNED_ORDERS_KEY = "industrilasappen.planned-orders.v1";
const PLANNER_CONFIG_KEY = "industrilasappen.planner-config.v1";
const DATA_VERSION = 4;

const $ = (selector) => document.querySelector(selector);
const $$ = (selector) => [...document.querySelectorAll(selector)];

const dom = {
  setupView: $("#setupView"),
  activeView: $("#activeView"),
  orderForm: $("#orderForm"),
  orderFormError: $("#orderFormError"),
  momentEditorList: $("#momentEditorList"),
  momentOverviewList: $("#momentOverviewList"),
  settingsDialog: $("#settingsDialog"),
  settingsError: $("#settingsError"),
  memberDialog: $("#memberDialog"),
  memberError: $("#memberError"),
  looseDialog: $("#looseDialog"),
  looseError: $("#looseError"),
  orderMenuDialog: $("#orderMenuDialog"),
  plannerView: $("#plannerView"),
  plannerOrderForm: $("#plannerOrderForm"),
  plannerOrderError: $("#plannerOrderError"),
  plannerOrderList: $("#plannerOrderList"),
  plannerScheduleList: $("#plannerScheduleList"),
  toast: $("#toast"),
};

const state = {
  settings: loadSettings(),
  order: loadOrder(),
  presets: loadPresets(),
  plannedOrders: loadPlannedOrders(),
  plannerConfig: loadPlannerConfig(),
  plannerEditingId: null,
  plannerEditingUpdatedAt: null,
  currentView: "planner",
  latestPlan: null,
  pendingPlannedOrderId: null,
  pendingPlannedUpdatedAt: null,
  toastTimer: null,
};

function loadSettings() {
  try {
    const saved = JSON.parse(localStorage.getItem(SETTINGS_KEY));
    if (!saved) return cloneDefaultSettings();
    const merged = { ...cloneDefaultSettings(), ...saved };
    return settingsAreValid(merged).valid ? merged : cloneDefaultSettings();
  } catch {
    return cloneDefaultSettings();
  }
}

function normalizeMoment(moment, fallbackName = "Montering") {
  const maxStaff = Math.floor(Number(moment?.maxStaff));
  const resourceName = String(moment?.resourceName || "").trim();
  const resourceCapacity = Math.floor(Number(moment?.resourceCapacity));
  return {
    id: moment?.id || id(),
    plannedMomentId: moment?.plannedMomentId || null,
    name: String(moment?.name || fallbackName).trim() || fallbackName,
    leadTime: Number(moment?.leadTime) > 0 ? Number(moment.leadTime) : 45,
    leadUnit: moment?.leadUnit === "minutes" ? "minutes" : "seconds",
    maxStaff: Number.isFinite(maxStaff) && maxStaff > 0 ? maxStaff : null,
    resourceName,
    resourceCapacity: resourceName ? (Number.isFinite(resourceCapacity) && resourceCapacity > 0 ? resourceCapacity : 1) : null,
    preCompletedCartons: Math.max(0, Math.floor(Number(moment?.preCompletedCartons) || 0)),
    preCompletedLoose: Math.max(0, Math.floor(Number(moment?.preCompletedLoose) || 0)),
    events: Array.isArray(moment?.events) ? moment.events : [],
  };
}

function migrateOrder(saved) {
  if (!saved || !saved.quantity || !saved.perCarton) return null;

  if (Array.isArray(saved.moments) && saved.moments.length) {
    saved.dataVersion = DATA_VERSION;
    saved.moments = saved.moments.map((moment, index) => normalizeMoment(moment, `Moment ${index + 1}`));
    saved.members = Array.isArray(saved.members) ? saved.members : [];
    saved.pauseEvents = Array.isArray(saved.pauseEvents) ? saved.pauseEvents : [];
    saved.momentSessions = Array.isArray(saved.momentSessions) ? saved.momentSessions : [];
    saved.includeEveningShift = Boolean(saved.includeEveningShift);
    if (!saved.activeMomentId || !saved.moments.some((moment) => moment.id === saved.activeMomentId)) {
      saved.activeMomentId = firstIncompleteMoment(saved)?.id || saved.moments[0].id;
    }
    if (!saved.momentSessions.length && !saved.completedAt) {
      saved.momentSessions.push({ id: id(), momentId: saved.activeMomentId, start: saved.startAt, end: null });
    }
    return saved;
  }

  // Migrera den äldre modellen med ett enda moment utan att förlora en aktiv order.
  const moment = normalizeMoment({
    name: "Montering",
    leadTime: saved.leadTime,
    leadUnit: saved.leadUnit,
    preCompletedCartons: saved.preCompletedCartons,
    preCompletedLoose: saved.preCompletedLoose,
    events: saved.events,
  });
  return {
    ...saved,
    dataVersion: DATA_VERSION,
    moments: [moment],
    activeMomentId: moment.id,
    momentSessions: saved.completedAt ? [] : [{ id: id(), momentId: moment.id, start: saved.startAt, end: null }],
    members: Array.isArray(saved.members) ? saved.members : [],
    pauseEvents: Array.isArray(saved.pauseEvents) ? saved.pauseEvents : [],
    includeEveningShift: Boolean(saved.includeEveningShift),
  };
}

function loadOrder() {
  try {
    return migrateOrder(JSON.parse(localStorage.getItem(ORDER_KEY)));
  } catch {
    return null;
  }
}

function loadPresets() {
  try {
    const saved = JSON.parse(localStorage.getItem(PRESETS_KEY));
    if (!Array.isArray(saved)) return [];
    return saved
      .filter((preset) => preset && preset.id && preset.name && Array.isArray(preset.moments) && preset.moments.length)
      .map((preset) => ({
        ...preset,
        perCarton: Math.max(1, Math.floor(Number(preset.perCarton) || 50)),
        bufferPercent: Math.max(0, Number(preset.bufferPercent) || 0),
        includeEveningShift: Boolean(preset.includeEveningShift),
        moments: preset.moments.map((moment, index) => {
          const maxStaff = Math.floor(Number(moment.maxStaff));
          const resourceName = String(moment.resourceName || "").trim();
          const resourceCapacity = Math.floor(Number(moment.resourceCapacity));
          return {
            name: String(moment.name || `Moment ${index + 1}`),
            leadTime: Number(moment.leadTime) > 0 ? Number(moment.leadTime) : 45,
            leadUnit: moment.leadUnit === "minutes" ? "minutes" : "seconds",
            maxStaff: Number.isFinite(maxStaff) && maxStaff > 0 ? maxStaff : null,
            resourceName,
            resourceCapacity: resourceName ? (Number.isFinite(resourceCapacity) && resourceCapacity > 0 ? resourceCapacity : 1) : null,
          };
        }),
      }));
  } catch {
    return [];
  }
}


function normalizePlannedOrder(order) {
  return normalizePlanningOrder(order);
}

function loadPlannedOrders() {
  try {
    const saved = JSON.parse(localStorage.getItem(PLANNED_ORDERS_KEY));
    if (!Array.isArray(saved)) return [];
    return saved.map(normalizePlannedOrder).filter(Boolean);
  } catch {
    return [];
  }
}

function loadPlannerConfig() {
  try { return normalizePlannerConfig(JSON.parse(localStorage.getItem(PLANNER_CONFIG_KEY)) || {}); }
  catch { return normalizePlannerConfig(); }
}

function savePlannedOrders() {
  localStorage.setItem(PLANNED_ORDERS_KEY, JSON.stringify(state.plannedOrders));
}

function savePlannerConfig() {
  localStorage.setItem(PLANNER_CONFIG_KEY, JSON.stringify(state.plannerConfig));
}

function saveSettings() {
  localStorage.setItem(SETTINGS_KEY, JSON.stringify(state.settings));
}

function saveOrder() {
  if (state.order) localStorage.setItem(ORDER_KEY, JSON.stringify(state.order));
  else localStorage.removeItem(ORDER_KEY);
}

function savePresets() {
  localStorage.setItem(PRESETS_KEY, JSON.stringify(state.presets));
}

function id() {
  return globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(16).slice(2)}`;
}

function parseDecimal(value) {
  const normalized = String(value ?? "").trim().replace(/\s/g, "").replace(",", ".");
  const number = Number(normalized);
  return Number.isFinite(number) ? number : NaN;
}

function toLocalInputValue(value = new Date()) {
  const date = new Date(value);
  const offset = date.getTimezoneOffset() * 60_000;
  return new Date(date.getTime() - offset).toISOString().slice(0, 16);
}

function escapeHtml(value) {
  return String(value ?? "")
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
    .replaceAll('"', "&quot;")
    .replaceAll("'", "&#039;");
}

function formatNumber(value) {
  return new Intl.NumberFormat("sv-SE", { maximumFractionDigits: 1 }).format(value);
}

function formatDuration(secondsValue, compact = false) {
  const seconds = Math.max(0, Number(secondsValue) || 0);
  if (seconds > 0 && seconds < 60) return `${Math.max(1, Math.round(seconds))} sek`;
  const minutes = Math.round(seconds / 60);
  const days = Math.floor(minutes / (24 * 60));
  const hours = Math.floor((minutes % (24 * 60)) / 60);
  const mins = minutes % 60;
  const parts = [];
  if (days) parts.push(`${days} d`);
  if (hours) parts.push(`${hours} h`);
  if (mins || !parts.length) parts.push(`${mins} min`);
  return parts.slice(0, compact ? 2 : 3).join(" ");
}

function formatPieceTime(secondsValue) {
  const seconds = Math.max(0, Number(secondsValue) || 0);
  if (seconds < 60) return `${formatNumber(seconds)} sek/st`;
  const wholeMinutes = Math.floor(seconds / 60);
  const remainingSeconds = Math.round(seconds % 60);
  return remainingSeconds ? `${wholeMinutes} min ${remainingSeconds} sek/st` : `${wholeMinutes} min/st`;
}

function isSameDay(left, right) {
  return left.getFullYear() === right.getFullYear() && left.getMonth() === right.getMonth() && left.getDate() === right.getDate();
}

function formatFinish(value) {
  if (!value) return "—";
  const date = new Date(value);
  if (!Number.isFinite(date.getTime())) return "—";
  const now = new Date();
  const tomorrow = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1);
  const time = new Intl.DateTimeFormat("sv-SE", { hour: "2-digit", minute: "2-digit" }).format(date);
  if (isSameDay(date, now)) return `I dag ${time}`;
  if (isSameDay(date, tomorrow)) return `I morgon ${time}`;
  const day = new Intl.DateTimeFormat("sv-SE", { weekday: "short", day: "numeric", month: "short" }).format(date);
  return `${day} ${time}`;
}

function formatTimestamp(value) {
  const date = new Date(value);
  if (!Number.isFinite(date.getTime())) return "—";
  return new Intl.DateTimeFormat("sv-SE", {
    weekday: "short",
    day: "numeric",
    month: "short",
    hour: "2-digit",
    minute: "2-digit",
  }).format(date);
}

function initials(name) {
  const parts = String(name || "M").trim().split(/\s+/).filter(Boolean);
  return parts.slice(0, 2).map((part) => part[0]).join("").toUpperCase() || "M";
}

function showError(element, message = "") {
  element.textContent = message;
  element.hidden = !message;
}

function toast(message) {
  clearTimeout(state.toastTimer);
  dom.toast.textContent = message;
  dom.toast.classList.add("visible");
  state.toastTimer = setTimeout(() => dom.toast.classList.remove("visible"), 2600);
}

function scheduleSettings(includeEveningShift = false) {
  return { ...state.settings, includeEveningShift: Boolean(includeEveningShift) };
}

function scheduleSettingsForOrder(order = state.order) {
  return scheduleSettings(Boolean(order?.includeEveningShift));
}

function momentOrderShape(order, moment) {
  return {
    quantity: order.quantity,
    perCarton: order.perCarton,
    preCompletedCartons: moment.preCompletedCartons,
    preCompletedLoose: moment.preCompletedLoose,
    events: moment.events || [],
    leadTime: moment.leadTime,
    leadUnit: moment.leadUnit,
    bufferPercent: order.bufferPercent,
  };
}

function completedQuantityForMoment(order, moment) {
  return totalCompletedQuantity(momentOrderShape(order, moment));
}

function completedAfterStartForMoment(order, moment) {
  const shape = momentOrderShape(order, moment);
  const baseline = preCompletedQuantity(shape);
  const capacity = Math.max(0, order.quantity - baseline);
  return Math.min(capacity, registeredProductionQuantity(shape));
}

function preCompletedDescription(order, moment) {
  const cartons = Math.max(0, Math.floor(Number(moment?.preCompletedCartons) || 0));
  const loose = Math.max(0, Math.floor(Number(moment?.preCompletedLoose) || 0));
  const total = preCompletedQuantity(momentOrderShape(order, moment));
  if (!total) return "0 st";
  const parts = [];
  if (cartons) parts.push(`${cartons} ${cartons === 1 ? "kartong" : "kartonger"}`);
  if (loose) parts.push(`${loose} lösa st`);
  return `${parts.join(" + ")} (${formatNumber(total)} st)`;
}

function registeredCartons(moment) {
  return (moment?.events || []).filter((event) => event.type === "carton").length;
}

function allRegistrations(order = state.order) {
  return (order?.moments || []).flatMap((moment) => (moment.events || []).map((event) => ({ moment, event })));
}

function latestRegistration(order = state.order) {
  return allRegistrations(order).sort((left, right) => new Date(right.event.at) - new Date(left.event.at))[0] || null;
}

function ongoingMembers(order = state.order) {
  return (order?.members || []).filter((member) => !member.leftAt);
}

function activeMoment(order = state.order) {
  if (!order) return null;
  return order.moments.find((moment) => moment.id === order.activeMomentId) || order.moments[0] || null;
}

function firstIncompleteMoment(order) {
  return (order?.moments || []).find((moment) => completedQuantityForMoment(order, moment) < order.quantity) || null;
}

function allMomentsComplete(order) {
  return (order?.moments || []).length > 0 && order.moments.every((moment) => completedQuantityForMoment(order, moment) >= order.quantity);
}

function closeOpenMomentSession(order, at = new Date().toISOString()) {
  const session = [...(order.momentSessions || [])].reverse().find((item) => !item.end);
  if (session) session.end = at;
}

function openMomentSession(order, momentId, at = new Date().toISOString()) {
  if (!momentId || order.completedAt) return;
  order.momentSessions = Array.isArray(order.momentSessions) ? order.momentSessions : [];
  const open = [...order.momentSessions].reverse().find((item) => !item.end);
  if (open?.momentId === momentId) return;
  if (open) open.end = at;
  order.momentSessions.push({ id: id(), momentId, start: at, end: null });
}

function switchActiveMoment(momentId, { at = new Date().toISOString(), silent = false } = {}) {
  const order = state.order;
  const target = order?.moments.find((moment) => moment.id === momentId);
  if (!order || !target || target.id === order.activeMomentId) return;
  if (completedQuantityForMoment(order, target) >= order.quantity) return;
  closeOpenMomentSession(order, at);
  order.activeMomentId = target.id;
  openMomentSession(order, target.id, at);
  saveOrder();
  renderActiveOrder();
  if (!silent) toast(`${target.name} är nu aktivt moment`);
}

function personSecondsWorked(order, end = new Date()) {
  const requestedEnd = new Date(end);
  const completedEnd = order.completedAt ? new Date(order.completedAt) : null;
  const effectiveEnd = completedEnd && completedEnd < requestedEnd ? completedEnd : requestedEnd;
  return (order.members || []).reduce((total, member) => {
    const memberStart = new Date(member.joinedAt);
    const leftAt = member.leftAt ? new Date(member.leftAt) : null;
    const memberEnd = leftAt && leftAt < effectiveEnd ? leftAt : effectiveEnd;
    return total + productiveSecondsBetween(memberStart, memberEnd, scheduleSettingsForOrder(order), order.pauseEvents || []);
  }, 0);
}

function personSecondsWorkedForMoment(order, momentId, end = new Date()) {
  const requestedEnd = new Date(end);
  const completedEnd = order.completedAt ? new Date(order.completedAt) : null;
  const effectiveEnd = completedEnd && completedEnd < requestedEnd ? completedEnd : requestedEnd;
  const sessions = (order.momentSessions || []).filter((session) => session.momentId === momentId);
  const moment = (order.moments || []).find((item) => item.id === momentId);
  const limit = momentStaffLimit(moment);

  return sessions.reduce((sessionTotal, session) => {
    const sessionStart = new Date(session.start);
    const rawEnd = session.end ? new Date(session.end) : effectiveEnd;
    const sessionEnd = rawEnd < effectiveEnd ? rawEnd : effectiveEnd;
    if (!(sessionEnd > sessionStart)) return sessionTotal;

    const boundaries = [sessionStart.getTime(), sessionEnd.getTime()];
    for (const member of order.members || []) {
      const joined = new Date(member.joinedAt).getTime();
      const left = member.leftAt ? new Date(member.leftAt).getTime() : sessionEnd.getTime();
      if (joined > sessionStart.getTime() && joined < sessionEnd.getTime()) boundaries.push(joined);
      if (left > sessionStart.getTime() && left < sessionEnd.getTime()) boundaries.push(left);
    }
    boundaries.sort((a, b) => a - b);
    const unique = [...new Set(boundaries)];
    let total = 0;
    for (let index = 0; index < unique.length - 1; index += 1) {
      const intervalStart = new Date(unique[index]);
      const intervalEnd = new Date(unique[index + 1]);
      if (!(intervalEnd > intervalStart)) continue;
      const activeMembers = (order.members || []).filter((member) => {
        const memberStart = new Date(member.joinedAt);
        const memberEnd = member.leftAt ? new Date(member.leftAt) : effectiveEnd;
        return memberStart < intervalEnd && memberEnd > intervalStart;
      }).length;
      const countedMembers = Math.min(activeMembers, Number.isFinite(limit) ? limit : activeMembers);
      if (!countedMembers) continue;
      total += productiveSecondsBetween(intervalStart, intervalEnd, scheduleSettingsForOrder(order), order.pauseEvents || []) * countedMembers;
    }
    return sessionTotal + total;
  }, 0);
}

function cartonDetailText(remaining, perCarton) {
  const boxes = cartonBreakdown(remaining, perCarton);
  if (!boxes.total) return "Momentet är färdigt";
  if (!boxes.partialPieces) return boxes.total === 1 ? "1 hel kartong" : `${boxes.total} hela kartonger`;
  if (!boxes.full) return `Delkartong med ${boxes.partialPieces} st`;
  return `${boxes.full} hela + ${boxes.partialPieces} st`;
}

function momentStaffLimit(moment) {
  const limits = [];
  const maxStaff = Math.floor(Number(moment?.maxStaff));
  const resourceCapacity = Math.floor(Number(moment?.resourceCapacity));
  if (Number.isFinite(maxStaff) && maxStaff > 0) limits.push(maxStaff);
  if (String(moment?.resourceName || "").trim() && Number.isFinite(resourceCapacity) && resourceCapacity > 0) limits.push(resourceCapacity);
  return limits.length ? Math.min(...limits) : Infinity;
}

function momentCapacityText(moment) {
  const parts = [];
  const maxStaff = Math.floor(Number(moment?.maxStaff));
  const resourceName = String(moment?.resourceName || "").trim();
  const resourceCapacity = Math.floor(Number(moment?.resourceCapacity));
  if (Number.isFinite(maxStaff) && maxStaff > 0) parts.push(`max ${maxStaff} ${maxStaff === 1 ? "montör" : "montörer"}`);
  if (resourceName) parts.push(`${resourceName}: ${Number.isFinite(resourceCapacity) && resourceCapacity > 0 ? resourceCapacity : 1} st`);
  return parts.length ? parts.join(" · ") : "ingen särskild kapacitetsgräns";
}

function forecastForMoment(order, moment, now = new Date()) {
  const shape = momentOrderShape(order, moment);
  const done = completedQuantityForMoment(order, moment);
  const remaining = Math.max(0, order.quantity - done);
  const plan = calculateOrderPlan(shape);
  const workedPersonSeconds = personSecondsWorkedForMoment(order, moment.id, now);
  const measuredDone = completedAfterStartForMoment(order, moment);
  const measuredPieceSeconds = measuredDone > 0 ? workedPersonSeconds / measuredDone : 0;
  const hasMeasuredPace = measuredDone > 0 && workedPersonSeconds >= 30;
  const theoreticalLabor = plannedRemainingSeconds(shape, done);
  const remainingLabor = hasMeasuredPace ? measuredPieceSeconds * remaining : theoreticalLabor;
  return {
    moment,
    done,
    remaining,
    plan,
    workedPersonSeconds,
    measuredDone,
    measuredPieceSeconds,
    hasMeasuredPace,
    theoreticalLabor,
    remainingLabor,
  };
}

function forecastForOrder(order) {
  const now = new Date();
  const members = ongoingMembers(order);
  const teamCount = members.length;
  const momentForecasts = order.moments.map((moment) => forecastForMoment(order, moment, now));
  const remainingLabor = momentForecasts.reduce((sum, item) => sum + item.remainingLabor, 0);
  const startPoint = new Date(Math.max(now.getTime(), new Date(order.startAt).getTime()));
  const complete = momentForecasts.every((item) => item.remaining === 0);
  let wallSeconds = 0;
  let finish = null;
  if (complete) finish = new Date(order.completedAt || now);
  else if (!order.pausedAt && teamCount) {
    let cursor = new Date(startPoint);
    for (const item of momentForecasts) {
      if (!(item.remainingLabor > 0)) continue;
      const limit = momentStaffLimit(item.moment);
      const effectiveStaff = Math.max(1, Math.min(teamCount, Number.isFinite(limit) ? limit : teamCount));
      const momentWallSeconds = item.remainingLabor / effectiveStaff;
      wallSeconds += momentWallSeconds;
      cursor = addWorkingSeconds(cursor, momentWallSeconds, scheduleSettingsForOrder(order));
    }
    finish = cursor;
  }

  const progress = weightedMomentProgress({
    quantity: order.quantity,
    moments: order.moments,
    bufferPercent: order.bufferPercent,
    completedForMoment: (moment) => completedQuantityForMoment(order, moment),
  });

  return {
    now,
    teamCount,
    momentForecasts,
    remainingLabor,
    wallSeconds,
    finish,
    complete,
    progress,
    workedPersonSeconds: momentForecasts.reduce((sum, item) => sum + item.workedPersonSeconds, 0),
    completedMomentCount: momentForecasts.filter((item) => item.remaining === 0).length,
  };
}

function createEditorMoment(data = {}, index = 0) {
  const maxStaff = Math.floor(Number(data.maxStaff));
  const resourceName = String(data.resourceName || "");
  const resourceCapacity = Math.floor(Number(data.resourceCapacity));
  return {
    uiId: data.uiId || id(),
    name: data.name ?? (index === 0 ? "Montering" : `Moment ${index + 1}`),
    leadTime: data.leadTime ?? 45,
    leadUnit: data.leadUnit === "minutes" ? "minutes" : "seconds",
    maxStaff: Number.isFinite(maxStaff) && maxStaff > 0 ? maxStaff : null,
    resourceName,
    resourceCapacity: resourceName.trim() ? (Number.isFinite(resourceCapacity) && resourceCapacity > 0 ? resourceCapacity : 1) : null,
    preCompletedCartons: data.preCompletedCartons ?? 0,
    preCompletedLoose: data.preCompletedLoose ?? 0,
  };
}

function renderMomentEditors(moments) {
  const normalized = (moments?.length ? moments : [createEditorMoment()]).map((moment, index) => createEditorMoment(moment, index));
  dom.momentEditorList.innerHTML = normalized.map((moment, index) => `
    <article class="moment-editor-card" data-moment-editor="${escapeHtml(moment.uiId)}">
      <div class="moment-editor-header">
        <span class="moment-number">${index + 1}</span>
        <div class="field moment-name-field">
          <label>Momentnamn</label>
          <input type="text" value="${escapeHtml(moment.name)}" data-moment-name autocomplete="off" placeholder="Exempel: Förmontering" />
        </div>
        <button class="remove-moment" type="button" data-remove-moment aria-label="Ta bort delmoment" ${normalized.length === 1 ? "disabled" : ""}>×</button>
      </div>
      <div class="moment-editor-grid">
        <div class="field">
          <label>Originalledtid per styck</label>
          <div class="segmented-input">
            <input type="text" inputmode="decimal" value="${escapeHtml(String(moment.leadTime).replace(".", ","))}" data-moment-lead aria-label="Ledtid per styck" />
            <select data-moment-unit aria-label="Enhet för ledtid">
              <option value="seconds" ${moment.leadUnit === "seconds" ? "selected" : ""}>sekunder</option>
              <option value="minutes" ${moment.leadUnit === "minutes" ? "selected" : ""}>minuter</option>
            </select>
          </div>
        </div>
        <div class="moment-pre-grid">
          <div class="field">
            <label>Färdiga kartonger före start</label>
            <div class="input-suffix"><input type="number" min="0" step="1" inputmode="numeric" value="${moment.preCompletedCartons}" data-moment-pre-cartons /><span>st</span></div>
          </div>
          <div class="field">
            <label>Lösa stycken före start</label>
            <div class="input-suffix"><input type="number" min="0" step="1" inputmode="numeric" value="${moment.preCompletedLoose}" data-moment-pre-loose /><span>st</span></div>
          </div>
        </div>
      </div>
      <div class="moment-capacity-box">
        <div class="moment-capacity-heading">
          <strong>Kapacitet i momentet</strong>
          <span>För orderplanering och prognos</span>
        </div>
        <div class="moment-capacity-grid">
          <div class="field">
            <label>Max montörer samtidigt</label>
            <div class="input-suffix"><input type="number" min="1" step="1" inputmode="numeric" value="${moment.maxStaff ?? ""}" placeholder="Alla" data-moment-max-staff /><span>pers</span></div>
            <p class="field-help">Tomt = ingen särskild bemanningsgräns.</p>
          </div>
          <div class="field">
            <label>Begränsande resurs <span class="optional-label">valfritt</span></label>
            <input type="text" value="${escapeHtml(moment.resourceName)}" data-moment-resource autocomplete="off" placeholder="Exempel: Press 1 eller Fixtur A" />
            <p class="field-help">Samma namn i flera presets behandlas som samma resurs.</p>
          </div>
          <div class="field">
            <label>Antal tillgängliga resurser</label>
            <div class="input-suffix"><input type="number" min="1" step="1" inputmode="numeric" value="${moment.resourceName.trim() ? (moment.resourceCapacity ?? 1) : ""}" placeholder="1" data-moment-resource-capacity /><span>st</span></div>
            <p class="field-help">Används bara när en resurs är angiven.</p>
          </div>
        </div>
      </div>
    </article>`).join("");
}

function collectMomentEditors() {
  return $$('[data-moment-editor]').map((card, index) => {
    const maxStaffRaw = card.querySelector("[data-moment-max-staff]").value.trim();
    const resourceName = card.querySelector("[data-moment-resource]").value.trim();
    const resourceCapacityRaw = card.querySelector("[data-moment-resource-capacity]").value.trim();
    return {
      uiId: card.dataset.momentEditor,
      name: card.querySelector("[data-moment-name]").value.trim() || `Moment ${index + 1}`,
      leadTime: parseDecimal(card.querySelector("[data-moment-lead]").value),
      leadUnit: card.querySelector("[data-moment-unit]").value,
      maxStaff: maxStaffRaw ? Number(maxStaffRaw) : null,
      resourceName,
      resourceCapacity: resourceName ? (resourceCapacityRaw ? Number(resourceCapacityRaw) : 1) : null,
      preCompletedCartons: Number(card.querySelector("[data-moment-pre-cartons]").value),
      preCompletedLoose: Number(card.querySelector("[data-moment-pre-loose]").value),
    };
  });
}

function validateMomentEditors(quantity, perCarton, moments = collectMomentEditors()) {
  if (!moments.length) return "Lägg till minst ett delmoment.";
  for (let index = 0; index < moments.length; index += 1) {
    const moment = moments[index];
    if (!(moment.leadTime > 0)) return `Ledtiden för ${moment.name || `moment ${index + 1}`} måste vara större än noll.`;
    if (moment.maxStaff !== null && (!Number.isInteger(moment.maxStaff) || moment.maxStaff < 1)) return `Max montörer för ${moment.name} måste vara minst 1 eller lämnas tomt.`;
    if (moment.resourceName && (!Number.isInteger(moment.resourceCapacity) || moment.resourceCapacity < 1)) return `Antal tillgängliga resurser för ${moment.name} måste vara minst 1.`;
    if (!Number.isInteger(moment.preCompletedCartons) || moment.preCompletedCartons < 0) return `Ange ett giltigt antal färdiga kartonger för ${moment.name}.`;
    if (!Number.isInteger(moment.preCompletedLoose) || moment.preCompletedLoose < 0) return `Ange ett giltigt antal lösa stycken för ${moment.name}.`;
    if (moment.preCompletedCartons * perCarton + moment.preCompletedLoose > quantity) return `Färdigt före start för ${moment.name} kan inte vara större än orderantalet.`;
  }
  return "";
}

function updatePreview() {
  const quantity = Number($("#totalQuantity").value);
  const perCarton = Number($("#perCarton").value);
  const bufferPercent = parseDecimal($("#bufferPercent").value);
  const includeEveningShift = $("#includeEveningShift").checked;
  const start = new Date($("#startDateTime").value);
  const moments = collectMomentEditors();
  const momentError = validateMomentEditors(quantity, perCarton, moments);

  if (!(Number.isInteger(quantity) && quantity > 0 && Number.isInteger(perCarton) && perCarton > 0 && bufferPercent >= 0 && Number.isFinite(start.getTime())) || momentError) {
    $("#previewFinish").textContent = "—";
    $("#previewCartons").textContent = moments.length ? String(moments.length) : "—";
    $("#previewBaseTime").textContent = "—";
    $("#previewBufferedTime").textContent = "—";
    return;
  }

  let originalRemaining = 0;
  let bufferedRemaining = 0;
  for (const moment of moments) {
    const shape = {
      quantity,
      perCarton,
      preCompletedCartons: moment.preCompletedCartons,
      preCompletedLoose: moment.preCompletedLoose,
      events: [],
      leadTime: moment.leadTime,
      leadUnit: moment.leadUnit,
      bufferPercent,
    };
    const done = preCompletedQuantity(shape);
    const plan = calculateOrderPlan(shape);
    const ratio = quantity ? Math.max(0, quantity - done) / quantity : 0;
    originalRemaining += plan.originalTotalSeconds * ratio;
    bufferedRemaining += plan.plannedTotalSeconds * ratio;
  }

  const finish = bufferedRemaining ? addWorkingSeconds(start, bufferedRemaining, scheduleSettings(includeEveningShift)) : start;
  $("#previewFinish").textContent = bufferedRemaining ? formatFinish(finish) : "Redan färdig";
  $("#previewCartons").textContent = String(moments.length);
  $("#previewBaseTime").textContent = formatDuration(originalRemaining, true);
  $("#previewBufferedTime").textContent = formatDuration(bufferedRemaining, true);
  $("#previewTrack").style.width = `${Math.max(10, Math.min(100, 100 / (1 + bufferPercent / 100)))}%`;
  const fridayText = state.settings.workdays.map(Number).includes(5)
    ? ` · fre till ${state.settings.fridayEnd || "14:00"}${state.settings.fridayNoLunch === false ? "" : " utan lunchrast"}`
    : "";
  const eveningText = includeEveningShift ? ` + kväll mån–tors ${state.settings.eveningStart || "16:00"}–${state.settings.eveningEnd || "01:00"}` : "";
  $("#previewSchedule").textContent = `Dag ${state.settings.dayStart}–${state.settings.dayEnd}${fridayText}${eveningText} · raster hoppas över`;
}

function validateNewOrder() {
  const quantity = Number($("#totalQuantity").value);
  const perCarton = Number($("#perCarton").value);
  const bufferPercent = parseDecimal($("#bufferPercent").value);
  const start = new Date($("#startDateTime").value);

  if (!Number.isInteger(quantity) || quantity < 1) return "Ange ett giltigt antal på ordern.";
  if (!Number.isInteger(perCarton) || perCarton < 1) return "Ange ett giltigt antal per kartong.";
  const momentError = validateMomentEditors(quantity, perCarton);
  if (momentError) return momentError;
  if (!(bufferPercent >= 0 && bufferPercent <= 500)) return "Ange ett giltigt tillägg mellan 0 och 500 %.";
  if (!Number.isFinite(start.getTime())) return "Ange när ordern startar.";
  return "";
}

function startOrder(event) {
  event.preventDefault();
  const error = validateNewOrder();
  showError(dom.orderFormError, error);
  if (error) return;
  if (state.pendingPlannedOrderId) {
    const source = state.plannedOrders.find((order) => order.id === state.pendingPlannedOrderId);
    if (!source || source.updatedAt !== state.pendingPlannedUpdatedAt) {
      return showError(dom.orderFormError, "Planeringsordern har ändrats. Öppna den från planen igen för aktuellt kvarvarande arbete.");
    }
  }

  const startAt = new Date($("#startDateTime").value).toISOString();
  const quantity = Number($("#totalQuantity").value);
  const perCarton = Number($("#perCarton").value);
  const moments = collectMomentEditors().map((moment, index) => normalizeMoment({
    id: id(),
    plannedMomentId: state.pendingPlannedOrderId && state.plannedOrders.find((order) => order.id === state.pendingPlannedOrderId)?.moments.some((source) => source.id === moment.uiId) ? moment.uiId : null,
    name: moment.name || `Moment ${index + 1}`,
    leadTime: moment.leadTime,
    leadUnit: moment.leadUnit,
    maxStaff: moment.maxStaff,
    resourceName: moment.resourceName,
    resourceCapacity: moment.resourceCapacity,
    preCompletedCartons: moment.preCompletedCartons,
    preCompletedLoose: moment.preCompletedLoose,
    events: [],
  }, `Moment ${index + 1}`));

  state.order = {
    dataVersion: DATA_VERSION,
    id: id(),
    name: $("#orderName").value.trim(),
    quantity,
    perCarton,
    bufferPercent: parseDecimal($("#bufferPercent").value),
    includeEveningShift: $("#includeEveningShift").checked,
    startAt,
    createdAt: new Date().toISOString(),
    moments,
    activeMomentId: null,
    momentSessions: [],
    members: [{ id: id(), name: "Jag", isOwner: true, joinedAt: startAt, leftAt: null }],
    pauseEvents: [],
    pausedAt: null,
    completedAt: null,
  };

  const first = firstIncompleteMoment(state.order);
  state.order.activeMomentId = first?.id || moments[0].id;
  if (first) openMomentSession(state.order, first.id, startAt);
  else state.order.completedAt = startAt;

  if (state.pendingPlannedOrderId) {
    state.order.plannedOrderId = state.pendingPlannedOrderId;
    state.order.plannedUpdatedAt = state.plannedOrders.find((item) => item.id === state.pendingPlannedOrderId)?.updatedAt || null;
    state.pendingPlannedOrderId = null;
    state.pendingPlannedUpdatedAt = null;
  }
  saveOrder();
  render();
  toast(moments.length > 1 ? `Ordern är startad · ${moments.length} delmoment` : "Ordern är startad");
}

function renderMomentOverview(order, forecast) {
  const active = activeMoment(order);
  $("#activeMomentBadge").textContent = active ? `Aktivt: ${active.name}` : "—";
  dom.momentOverviewList.innerHTML = forecast.momentForecasts.map((item, index) => {
    const isActive = item.moment.id === order.activeMomentId;
    const pct = order.quantity ? Math.min(100, (item.done / order.quantity) * 100) : 0;
    const status = item.remaining === 0 ? "Klar" : isActive ? "Aktiv" : item.done > 0 ? "Påbörjad" : "Ej startad";
    const statusClass = item.remaining === 0 ? "good" : isActive ? "active" : "";
    const pace = item.hasMeasuredPace ? formatPieceTime(item.measuredPieceSeconds) : formatPieceTime(item.plan.pieceSeconds);
    return `
      <article class="moment-overview-item ${isActive ? "is-active" : ""}">
        <div class="moment-overview-main">
          <span class="moment-index">${index + 1}</span>
          <div class="moment-overview-copy">
            <div class="moment-overview-title"><strong>${escapeHtml(item.moment.name)}</strong><span class="moment-state ${statusClass}">${status}</span></div>
            <div class="mini-progress"><span style="width:${pct}%"></span></div>
            <small>${formatNumber(item.done)} / ${formatNumber(order.quantity)} st · ${Math.round(pct)} % · ${escapeHtml(pace)}</small>
            <small class="moment-capacity-note">Kapacitet: ${escapeHtml(momentCapacityText(item.moment))}</small>
          </div>
        </div>
        ${!isActive && item.remaining > 0 ? `<button class="quiet-button moment-switch" type="button" data-activate-moment="${escapeHtml(item.moment.id)}">Aktivera</button>` : ""}
      </article>`;
  }).join("");
}

function renderActiveOrder() {
  const order = state.order;
  if (!order) return;
  const forecast = forecastForOrder(order);
  const current = activeMoment(order) || firstIncompleteMoment(order) || order.moments[0];
  const activeForecast = forecast.momentForecasts.find((item) => item.moment.id === current?.id) || forecast.momentForecasts[0];
  const progress = forecast.progress * 100;
  const boxes = cartonBreakdown(activeForecast?.remaining || 0, order.perCarton);
  const multiPlan = calculateMultiMomentPlan({ quantity: order.quantity, moments: order.moments, bufferPercent: order.bufferPercent });

  $("#activeOrderTitle").textContent = order.name || "Aktiv order";
  $("#activeOrderEyebrow").textContent = order.pausedAt ? "Order pausad" : forecast.complete ? "Order färdig" : "Pågående order";
  $("#progressPercent").textContent = `${Math.round(progress)}%`;
  $("#progressFill").style.width = `${progress}%`;
  $("#progressBar").setAttribute("aria-valuenow", String(Math.round(progress)));
  $("#doneCount").textContent = `${forecast.completedMomentCount}/${order.moments.length}`;
  $("#remainingCount").textContent = String(order.moments.length - forecast.completedMomentCount);

  if (forecast.complete) {
    $("#forecastFinish").textContent = "Färdig";
    $("#forecastSource").textContent = order.completedAt ? formatTimestamp(order.completedAt) : "Alla delmoment är klara";
  } else if (order.pausedAt) {
    $("#forecastFinish").textContent = "Pausad";
    $("#forecastSource").textContent = "Fortsätt ordern för ny sluttid";
  } else if (!forecast.teamCount) {
    $("#forecastFinish").textContent = "Inget aktivt team";
    $("#forecastSource").textContent = "Lägg till en medarbetare";
  } else {
    $("#forecastFinish").textContent = formatFinish(forecast.finish);
    const measuredCount = forecast.momentForecasts.filter((item) => item.hasMeasuredPace && item.remaining > 0).length;
    $("#forecastSource").textContent = measuredCount ? "Uppmätt takt där underlag finns" : "Enligt delmomentens originaltider";
  }

  $("#timeRemaining").textContent = forecast.teamCount ? formatDuration(forecast.wallSeconds, true) : "—";
  $("#cartonsRemaining").textContent = String(boxes.total);
  $("#cartonDetail").textContent = current ? `${current.name} · ${cartonDetailText(activeForecast.remaining, order.perCarton)}` : "—";
  $("#activeTeamCount").textContent = String(forecast.teamCount);
  $("#cartonSizeLabel").textContent = `${formatNumber(order.perCarton)} st / kartong`;
  $("#registerMomentName").textContent = current?.name || "—";
  $("#paceMomentName").textContent = current?.name || "—";

  const paceElement = $("#paceConfidence");
  paceElement.className = "status-pill subtle";
  if (!activeForecast?.hasMeasuredPace) {
    paceElement.textContent = "Ej mätbar än";
    $("#teamPace").textContent = activeForecast?.measuredDone > 0 ? "Samlar arbetstid" : "Inväntar ny produktion";
  } else if (registeredCartons(current) < 3) {
    paceElement.textContent = "Preliminär";
    paceElement.classList.add("warn");
    $("#teamPace").textContent = formatPieceTime(activeForecast.measuredPieceSeconds);
  } else {
    paceElement.textContent = "Stabilare underlag";
    paceElement.classList.add("good");
    $("#teamPace").textContent = formatPieceTime(activeForecast.measuredPieceSeconds);
  }

  $("#originalPace").textContent = activeForecast ? formatPieceTime(activeForecast.plan.pieceSeconds) : "—";
  $("#actualPace").textContent = activeForecast?.hasMeasuredPace ? formatPieceTime(activeForecast.measuredPieceSeconds) : "—";
  $("#preCompletedDisplay").textContent = current ? preCompletedDescription(order, current) : "0 st";
  $("#completedCartons").textContent = current ? String(registeredCartons(current)) : "0";

  if (activeForecast?.hasMeasuredPace) {
    const plannedPieceAverage = activeForecast.plan.plannedTotalSeconds / order.quantity;
    const difference = ((activeForecast.measuredPieceSeconds - plannedPieceAverage) / plannedPieceAverage) * 100;
    if (Math.abs(difference) < 1) $("#paceDelta").textContent = "I nivå med plan";
    else $("#paceDelta").textContent = `${formatNumber(Math.abs(difference))} % ${difference < 0 ? "snabbare" : "långsammare"}`;
  } else {
    $("#paceDelta").textContent = "—";
  }

  $("#undoButton").disabled = !latestRegistration(order);
  const currentRemaining = activeForecast?.remaining || 0;
  $("#completeCartonButton").disabled = !currentRemaining || Boolean(order.pausedAt) || forecast.complete;
  $("#addLooseButton").disabled = !currentRemaining || Boolean(order.pausedAt) || forecast.complete;
  $("#pauseButton").textContent = order.pausedAt ? "Fortsätt order" : "Pausa order";
  $("#pauseButton").disabled = forecast.complete;

  renderMomentOverview(order, forecast);
  renderTeamList(forecast.now);
  renderOrderSummary(multiPlan, forecast);
}

function renderTeamList(now) {
  const order = state.order;
  const effectiveNow = order.completedAt && new Date(order.completedAt) < now ? new Date(order.completedAt) : now;
  $("#teamList").innerHTML = order.members.map((member) => {
    const active = !member.leftAt;
    const memberEnd = member.leftAt && new Date(member.leftAt) < effectiveNow ? member.leftAt : effectiveNow;
    const worked = productiveSecondsBetween(member.joinedAt, memberEnd, scheduleSettingsForOrder(order), order.pauseEvents || []);
    return `
      <div class="team-member">
        <span class="avatar" aria-hidden="true">${escapeHtml(initials(member.name))}</span>
        <span class="member-copy">
          <strong>${escapeHtml(member.name)}</strong>
          <small>${active ? `Aktiv · ${formatDuration(worked, true)}` : `Avslutad · ${formatDuration(worked, true)}`}</small>
        </span>
        ${active && !member.isOwner
          ? `<button class="member-stop" type="button" data-stop-member="${escapeHtml(member.id)}">Avsluta</button>`
          : `<span class="member-status">${active ? "Aktiv" : "Klar"}</span>`}
      </div>`;
  }).join("");
}

function renderOrderSummary(plan, forecast) {
  const momentRows = state.order.moments.map((moment) => {
    const item = forecast.momentForecasts.find((entry) => entry.moment.id === moment.id);
    return `<div class="summary-row"><span>${escapeHtml(moment.name)}</span><strong>${formatDuration(item.plan.originalTotalSeconds)} · ${Math.round((item.done / state.order.quantity) * 100)} %</strong></div>`;
  }).join("");
  $("#orderSummary").innerHTML = `
    <div class="summary-row"><span>Start</span><strong>${escapeHtml(formatTimestamp(state.order.startAt))}</strong></div>
    <div class="summary-row"><span>Orderantal</span><strong>${formatNumber(state.order.quantity)} st</strong></div>
    <div class="summary-row"><span>Delmoment</span><strong>${state.order.moments.length}</strong></div>
    ${momentRows}
    <div class="summary-row"><span>Kvällsskift</span><strong>${state.order.includeEveningShift ? "Ja · mån–tors 16:00–01:00" : "Nej"}</strong></div>
    <div class="summary-row"><span>Original totaltid</span><strong>${formatDuration(plan.originalTotalSeconds)}</strong></div>
    <div class="summary-row"><span>${formatNumber(state.order.bufferPercent)} % tillägg</span><strong>${formatDuration(plan.allowanceSeconds)}</strong></div>
    <div class="summary-row"><span>Planerad totaltid</span><strong>${formatDuration(plan.plannedTotalSeconds)}</strong></div>
    <div class="summary-row"><span>Registrerad persontid i moment</span><strong>${formatDuration(forecast.workedPersonSeconds)}</strong></div>`;
}

function render() {
  const plannerOpen = state.currentView === "planner";
  const hasOrder = Boolean(state.order);
  dom.plannerView.hidden = !plannerOpen;
  dom.setupView.hidden = plannerOpen || hasOrder;
  dom.activeView.hidden = plannerOpen || !hasOrder;
  const navButton = $("#plannerNavButton");
  if (navButton) navButton.textContent = plannerOpen ? "Orderkalkyl" : "Orderplanering";
  $("#syncPlanButton").hidden = !state.order?.plannedOrderId;
  if (plannerOpen) renderPlanner();
  else if (hasOrder) renderActiveOrder();
  else updatePreview();
}

function advanceAfterMomentCompletion(completedMoment, at) {
  const order = state.order;
  closeOpenMomentSession(order, at);
  const next = firstIncompleteMoment(order);
  if (next) {
    order.activeMomentId = next.id;
    openMomentSession(order, next.id, at);
    return next;
  }
  order.activeMomentId = completedMoment.id;
  order.completedAt = at;
  return null;
}

function completeCarton() {
  const order = state.order;
  const moment = activeMoment(order);
  if (!moment || order.pausedAt) return;
  const remaining = order.quantity - completedQuantityForMoment(order, moment);
  if (remaining <= 0) return;
  const quantity = Math.min(order.perCarton, remaining);
  const at = new Date().toISOString();
  moment.events.push({ id: id(), type: "carton", quantity, at });
  const next = quantity === remaining ? advanceAfterMomentCompletion(moment, at) : null;
  saveOrder();
  renderActiveOrder();
  navigator.vibrate?.(35);
  if (next) toast(`${moment.name} klart · nästa: ${next.name}`);
  else if (allMomentsComplete(order)) toast("Alla delmoment är klara");
  else toast(quantity < order.perCarton ? `Sista delkartongen: ${quantity} st` : `${moment.name}: kartong ${registeredCartons(moment)} registrerad`);
}

function addLoosePieces() {
  const order = state.order;
  const moment = activeMoment(order);
  const quantity = Number($("#looseQuantity").value);
  const remaining = moment ? order.quantity - completedQuantityForMoment(order, moment) : 0;
  if (!Number.isInteger(quantity) || quantity < 1) {
    showError(dom.looseError, "Ange minst ett färdigt stycke.");
    return;
  }
  if (quantity > remaining) {
    showError(dom.looseError, `Det återstår bara ${remaining} stycken i ${moment.name}.`);
    return;
  }
  const at = new Date().toISOString();
  moment.events.push({ id: id(), type: "loose", quantity, at });
  const next = quantity === remaining ? advanceAfterMomentCompletion(moment, at) : null;
  saveOrder();
  dom.looseDialog.close();
  renderActiveOrder();
  if (next) toast(`${moment.name} klart · nästa: ${next.name}`);
  else if (allMomentsComplete(order)) toast("Alla delmoment är klara");
  else toast(`${quantity} lösa stycken registrerade i ${moment.name}`);
}

function undoLastRegistration() {
  const order = state.order;
  const latest = latestRegistration(order);
  if (!latest) return;
  const index = latest.moment.events.findIndex((event) => event.id === latest.event.id);
  if (index >= 0) latest.moment.events.splice(index, 1);
  const at = new Date().toISOString();
  order.completedAt = null;
  closeOpenMomentSession(order, at);
  order.activeMomentId = latest.moment.id;
  openMomentSession(order, latest.moment.id, at);
  saveOrder();
  renderActiveOrder();
  toast(`Senaste registreringen i ${latest.moment.name} är ångrad`);
}

function togglePause() {
  const order = state.order;
  if (!order || allMomentsComplete(order)) return;
  const now = new Date().toISOString();
  if (order.pausedAt) {
    const openPause = [...order.pauseEvents].reverse().find((pause) => !pause.end);
    if (openPause) openPause.end = now;
    order.pausedAt = null;
    toast("Ordern fortsätter");
  } else {
    order.pausedAt = now;
    order.pauseEvents.push({ id: id(), start: now, end: null });
    toast("Ordern är pausad");
  }
  saveOrder();
  renderActiveOrder();
}

function addMember() {
  const name = $("#memberName").value.trim() || `Medarbetare ${state.order.members.length}`;
  const joinedAt = new Date($("#memberStart").value);
  const orderStart = new Date(state.order.startAt);
  const now = new Date();

  if (!Number.isFinite(joinedAt.getTime())) {
    showError(dom.memberError, "Ange när medarbetaren började.");
    return;
  }
  if (joinedAt < orderStart) {
    showError(dom.memberError, "Starttiden kan inte vara före orderns starttid.");
    return;
  }
  if (joinedAt > now) {
    showError(dom.memberError, "Starttiden kan inte ligga i framtiden.");
    return;
  }

  state.order.members.push({ id: id(), name, isOwner: false, joinedAt: joinedAt.toISOString(), leftAt: null });
  saveOrder();
  dom.memberDialog.close();
  renderActiveOrder();
  toast(`${name} har lagts till`);
}

function stopMember(memberId) {
  const member = state.order?.members.find((item) => item.id === memberId);
  if (!member || member.leftAt || member.isOwner) return;
  member.leftAt = new Date().toISOString();
  saveOrder();
  renderActiveOrder();
  toast(`${member.name} är avslutad på ordern`);
}

function finishOrder() {
  if (!state.order) return;
  const message = allMomentsComplete(state.order)
    ? "Stäng den färdiga ordern och gå tillbaka till start?"
    : "Avsluta ordern? Den aktiva ordern tas bort från enheten. Exportera en säkerhetskopia först om du vill spara den.";
  if (!window.confirm(message)) return;
  state.order = null;
  saveOrder();
  setNewOrderDefaults();
  render();
  toast("Redo för en ny order");
}

function renderPresetOptions(selectedId = $("#presetSelect")?.value || "") {
  const select = $("#presetSelect");
  if (!select) return;
  select.innerHTML = '<option value="">Välj sparad preset…</option>' + state.presets
    .slice()
    .sort((a, b) => a.name.localeCompare(b.name, "sv"))
    .map((preset) => `<option value="${escapeHtml(preset.id)}" ${preset.id === selectedId ? "selected" : ""}>${escapeHtml(preset.name)}</option>`)
    .join("");
  const hasSelection = Boolean(select.value);
  $("#loadPresetButton").disabled = !hasSelection;
  $("#deletePresetButton").disabled = !hasSelection;
  if ($("#plannerPresetSelect")) renderPlannerPresetOptions($("#plannerPresetSelect").value);
}

function saveCurrentAsPreset() {
  const quantity = Number($("#totalQuantity").value);
  const perCarton = Number($("#perCarton").value);
  const moments = collectMomentEditors();
  const error = validateMomentEditors(Math.max(1, quantity || 1), perCarton, moments);
  if (!Number.isInteger(perCarton) || perCarton < 1 || error) {
    showError(dom.orderFormError, error || "Ange ett giltigt antal per kartong innan du sparar preset.");
    return;
  }
  const bufferPercent = parseDecimal($("#bufferPercent").value);
  if (!(bufferPercent >= 0 && bufferPercent <= 500)) {
    showError(dom.orderFormError, "Ange ett giltigt tillägg innan du sparar preset.");
    return;
  }
  const suggested = $("#orderName").value.trim() || moments.map((moment) => moment.name).join(" + ");
  const name = window.prompt("Namn på preset", suggested || "Ny preset")?.trim();
  if (!name) return;
  const existing = state.presets.find((preset) => preset.name.toLocaleLowerCase("sv") === name.toLocaleLowerCase("sv"));
  const preset = {
    id: existing?.id || id(),
    name,
    perCarton,
    bufferPercent,
    includeEveningShift: $("#includeEveningShift").checked,
    moments: moments.map((moment) => ({
      name: moment.name,
      leadTime: moment.leadTime,
      leadUnit: moment.leadUnit,
      maxStaff: moment.maxStaff,
      resourceName: moment.resourceName,
      resourceCapacity: moment.resourceCapacity,
    })),
    updatedAt: new Date().toISOString(),
  };
  if (existing) Object.assign(existing, preset);
  else state.presets.push(preset);
  savePresets();
  renderPresetOptions(preset.id);
  showError(dom.orderFormError, "");
  toast(existing ? "Preset uppdaterad" : "Preset sparad");
}

function loadSelectedPreset() {
  const preset = state.presets.find((item) => item.id === $("#presetSelect").value);
  if (!preset) return;
  $("#perCarton").value = String(preset.perCarton);
  $("#bufferPercent").value = String(preset.bufferPercent).replace(".", ",");
  $("#includeEveningShift").checked = Boolean(preset.includeEveningShift);
  renderMomentEditors(preset.moments.map((moment) => ({ ...moment, preCompletedCartons: 0, preCompletedLoose: 0 })));
  updatePreview();
  toast(`${preset.name} är inläst`);
}

function deleteSelectedPreset() {
  const preset = state.presets.find((item) => item.id === $("#presetSelect").value);
  if (!preset) return;
  if (!window.confirm(`Ta bort preset “${preset.name}”?`)) return;
  state.presets = state.presets.filter((item) => item.id !== preset.id);
  savePresets();
  renderPresetOptions();
  toast("Preset borttagen");
}


function plannerLaborSeconds(order) {
  return orderLaborSeconds(order);
}

function renderPlannerPresetOptions(selectedId = $("#plannerPresetSelect")?.value || "") {
  const select = $("#plannerPresetSelect");
  if (!select) return;
  select.innerHTML = '<option value="">Ange operationerna själv</option>' + state.presets.slice()
    .sort((a, b) => a.name.localeCompare(b.name, "sv"))
    .map((preset) => `<option value="${escapeHtml(preset.id)}" ${preset.id === selectedId ? "selected" : ""}>${escapeHtml(preset.name)}</option>`).join("");
  updatePlannerPresetSummary();
}

function updatePlannerPresetSummary() {
  const preset = state.presets.find((item) => item.id === $("#plannerPresetSelect").value);
  $("#plannerApplyPreset").disabled = !preset;
  $("#plannerPresetSummary").textContent = preset
    ? `${preset.moments.length} operationer i mallen. Hämta dem och kontrollera operationsnummer, arbetsplats och kvar att göra.`
    : "Du kan lägga upp ordern utan en produktmall. Befintliga operationer ändras först när du väljer Hämta operationer.";
}

function plannerOrderDraft() {
  return {
    ...state.plannedOrders.find((order) => order.id === state.plannerEditingId),
    id: state.plannerEditingId || id(),
    name: $("#plannerOrderName").value.trim(),
    articleNumber: $("#plannerArticle").value.trim(),
    description: $("#plannerDescription").value.trim(),
    orderType: $("#plannerOrderType").value,
    line: $("#plannerOrderLine").value,
    quantity: Number($("#plannerQuantity").value),
    perCarton: Number($("#plannerPerCarton").value),
    bufferPercent: parseDecimal($("#plannerBuffer").value),
    dueAt: $("#plannerDeadline").value || null,
    dueKind: $("#plannerDueKind").value,
    readiness: $("#plannerReadiness").value,
    priority: $("#plannerPriority").value,
    note: $("#plannerNote").value.trim(),
    presetId: $("#plannerPresetSelect").value,
    presetName: state.presets.find((preset) => preset.id === $("#plannerPresetSelect").value)?.name || "",
    moments: collectPlannerMoments(),
    dependencies: collectPlannerDependencies(),
    needsReview: false,
    updatedAt: new Date().toISOString(),
  };
}

function collectPlannerMoments() {
  return $$("[data-planner-moment]").map((card) => {
    const field = (name) => card.querySelector(`[data-pm="${name}"]`).value.trim();
    return {
      id: card.dataset.plannerMoment,
      opNumber: field("number"), name: field("name"),
      remainingQuantity: field("remaining") === "" ? Number($("#plannerQuantity").value) : Number(field("remaining")),
      leadTime: field("time") === "" ? null : parseDecimal(field("time")),
      leadUnit: field("unit"), resourceName: field("resource"),
      maxStaff: field("staff") === "" ? null : Number(field("staff")), resourceCapacity: Number(field("capacity")),
    };
  });
}

function renderPlannerMoments(moments = [{}]) {
  const options = (selected) => `<option value="seconds" ${selected !== "minutes" ? "selected" : ""}>sekunder</option><option value="minutes" ${selected === "minutes" ? "selected" : ""}>minuter</option>`;
  $("#plannerMomentList").innerHTML = moments.map((moment, index) => {
    const uid = escapeHtml(moment.id || id());
    const input = (label, key, value, attributes = 'type="text"') => `<div class="field"><label for="pm-${uid}-${key}">${label}</label><input id="pm-${uid}-${key}" data-pm="${key}" value="${escapeHtml(value ?? "")}" ${attributes} /></div>`;
    return `<article class="planning-operation" data-planner-moment="${uid}">
      <div class="planning-operation-heading"><strong>Steg ${index + 1}</strong><div>
        <button class="text-button" type="button" data-plan-move="-1" ${index === 0 ? "disabled" : ""} aria-label="Flytta steg ${index + 1} upp">↑</button>
        <button class="text-button" type="button" data-plan-move="1" ${index === moments.length - 1 ? "disabled" : ""} aria-label="Flytta steg ${index + 1} ner">↓</button>
        <button class="danger-link" type="button" data-plan-remove-moment ${moments.length === 1 ? "disabled" : ""}>Ta bort</button></div></div>
      <div class="field-row">${input("Op. nr (valfritt)", "number", moment.opNumber, 'type="text" inputmode="numeric" placeholder="10"')}${input("Arbete / moment", "name", moment.name, 'type="text" placeholder="Vad görs i detta moment?"')}</div>
      <div class="field-row">${input("Kvar att göra (st)", "remaining", moment.remainingQuantity, 'type="number" min="0" step="1" placeholder="Hela planerade körningen"')}
        <div class="field"><label for="pm-${uid}-time">Stycktid för 1 person</label><div class="segmented-input"><input id="pm-${uid}-time" data-pm="time" type="text" inputmode="decimal" value="${escapeHtml(String(moment.leadTime ?? "").replace(".", ","))}" placeholder="Kan lämnas tom" /><select data-pm="unit" aria-label="Enhet för stycktid">${options(moment.leadUnit)}</select></div></div>
      </div>
      ${input("Arbetsplats / planeringsresurs", "resource", moment.resourceName, 'type="text" list="plannerResourceOptions" placeholder="Välj eller skriv en arbetsplats"')}
      <div class="field-row">${input("Max montörer (tomt = hela teamet)", "staff", moment.maxStaff === null ? "" : (moment.maxStaff ?? 1), 'type="number" min="1" step="1"')}${input("Antal parallella resursplatser", "capacity", moment.resourceCapacity ?? 1, 'type="number" min="1" step="1"')}</div>
      <p class="field-help">En enskild nitmaskin = 1 resursplats. Fler platser gäller bara likvärdiga platser med samma resursnamn. Planen räknar en montör per plats.</p>
    </article>`;
  }).join("");
}

function collectPlannerDependencies() {
  return $$("[data-planning-dependency]").map((card) => ({
    orderId: card.querySelector("select").value,
    released: card.querySelector('input[type="checkbox"]').checked,
  }));
}

function renderPlannerDependencies(dependencies = []) {
  const candidates = state.plannedOrders.filter((order) => order.id !== state.plannerEditingId);
  $("#plannerDependencyList").innerHTML = dependencies.length ? dependencies.map((dependency, index) => `
    <div class="planning-dependency" data-planning-dependency>
      <div class="field"><label for="dependency-${index}">Behövs från order</label><select id="dependency-${index}"><option value="">Välj order…</option>
        ${dependency.orderId && !candidates.some((order) => order.id === dependency.orderId) ? `<option value="${escapeHtml(dependency.orderId)}" selected>Saknad order – välj på nytt</option>` : ""}
        ${candidates.map((order) => `<option value="${escapeHtml(order.id)}" ${order.id === dependency.orderId ? "selected" : ""}>${escapeHtml(order.name)} · ${escapeHtml(order.description || ORDER_TYPES[order.orderType])} · Lina ${order.line}</option>`).join("")}</select></div>
      <label class="inline-check"><input type="checkbox" ${dependency.released ? "checked" : ""} />Överlämning klar för denna körning</label>
      <button class="danger-link" type="button" data-plan-remove-dependency>Ta bort koppling</button>
    </div>`).join("") : '<p class="field-help">Ingen kopplad order. Arbete kan ändå stoppas eller markeras för kontroll ovan.</p>';
}

function applyPlannerPreset() {
  const preset = state.presets.find((item) => item.id === $("#plannerPresetSelect").value);
  if (!preset) return;
  renderPlannerMoments(preset.moments.map((moment) => ({ ...moment, id: id(), remainingQuantity: $("#plannerQuantity").value || "" })));
  $("#plannerPerCarton").value = String(preset.perCarton);
  $("#plannerBuffer").value = String(preset.bufferPercent).replace(".", ",");
}

function resetPlannerOrderForm() {
  state.plannerEditingId = null;
  state.plannerEditingUpdatedAt = null;
  dom.plannerOrderForm.reset();
  $("#plannerOrderLine").value = state.plannerConfig.line;
  $("#plannerBuffer").value = String(state.settings.bufferPercent).replace(".", ",");
  $("#plannerFormTitle").textContent = "Lägg till order";
  $("#savePlannedOrderButton").textContent = "Lägg till i plan";
  $("#cancelPlannerEditButton").hidden = true;
  showError(dom.plannerOrderError, "");
  renderPlannerPresetOptions();
  renderPlannerMoments();
  renderPlannerDependencies();
}

function plannerConfigFromInputs() {
  return normalizePlannerConfig({
    startAt: $("#plannerStartAt").value,
    dayStaff: Number($("#plannerDayStaff").value),
    eveningStaff: Number($("#plannerEveningStaff").value),
    line: $("#plannerLine").value,
  });
}

function syncPlannerConfigInputs() {
  $("#plannerStartAt").value = toLocalInputValue(state.plannerConfig.startAt);
  $("#plannerDayStaff").value = String(state.plannerConfig.dayStaff);
  $("#plannerEveningStaff").value = String(state.plannerConfig.eveningStaff);
  $("#plannerLine").value = state.plannerConfig.line;
}

function buildPlannerRecommendation() {
  return buildWorkshopPlan({ orders: state.plannedOrders, config: state.plannerConfig, settings: state.settings });
}

function plannerDateLabel(value) {
  const date = new Date(value);
  return new Intl.DateTimeFormat("sv-SE", { weekday: "short", day: "numeric", month: "short" }).format(date);
}

function plannerClock(value) {
  return new Intl.DateTimeFormat("sv-SE", { hour: "2-digit", minute: "2-digit" }).format(new Date(value));
}

function plannerMarginText(milliseconds) {
  if (milliseconds === null || milliseconds === undefined) return "Kunde inte schemaläggas";
  const duration = formatDuration(Math.abs(milliseconds) / 1000, true);
  if (Math.abs(milliseconds) < 60_000) return "Precis vid måldatum";
  return milliseconds >= 0 ? `${duration} marginal` : `${duration} efter måldatum`;
}

function plannerConcurrentGroups(schedule) {
  const segments = (schedule?.segments || [])
    .map((segment, index) => ({
      ...segment,
      _index: index,
      _start: new Date(segment.start).getTime(),
      _end: new Date(segment.end).getTime(),
    }))
    .filter((segment) => Number.isFinite(segment._start) && Number.isFinite(segment._end) && segment._end > segment._start);
  if (!segments.length) return [];

  // Dela tidslinjen vid varje verklig start/slutpunkt. Då hamnar alla arbeten
  // som faktiskt pågår samtidigt i samma visuella block, även när ett längre
  // moment överlappar flera kortare moment på andra order.
  const boundaries = [...new Set(segments.flatMap((segment) => [segment._start, segment._end]))].sort((a, b) => a - b);
  const slices = [];

  for (let index = 0; index < boundaries.length - 1; index += 1) {
    const startMs = boundaries[index];
    const endMs = boundaries[index + 1];
    if (!(endMs > startMs)) continue;

    const active = segments
      .filter((segment) => segment._start <= startMs && segment._end >= endMs)
      .sort((a, b) => a._index - b._index);
    if (!active.length) continue;

    const byShift = new Map();
    for (const segment of active) {
      const key = segment.shift || "day";
      if (!byShift.has(key)) byShift.set(key, []);
      byShift.get(key).push(segment);
    }

    for (const [shift, items] of byShift.entries()) {
      const signature = items
        .map((item) => [item.orderId, item.momentId, item.staff, item.resourceName || "", item.resourceCapacity || ""].join("|"))
        .sort()
        .join("||");
      const previous = slices[slices.length - 1];
      if (previous && previous.shift === shift && previous.signature === signature && previous.end.getTime() === startMs) {
        previous.end = new Date(endMs);
      } else {
        slices.push({
          start: new Date(startMs),
          end: new Date(endMs),
          shift,
          items,
          signature,
        });
      }
    }
  }

  return slices;
}

function plannerResultUsesEvening(result) {
  return Boolean(result?.segments?.some((segment) => segment.shift === "evening"));
}

function plannerEveningSummary(schedule) {
  const strategy = schedule.strategy || {};
  if (!schedule.rows.some((row) => !row.complete)) return { title: "–", detail: "Inga öppna order" };
  if (!state.plannerConfig.eveningStaff) return { title: "Ej tillgängligt", detail: "0 personer på kväll" };
  if (!strategy.eveningNeeded) return { title: "Ej föreslaget", detail: "Utifrån kända tider och måldatum" };
  const shifts = strategy.eveningShiftDates.length;
  return { title: `${shifts} ${shifts === 1 ? "kväll" : "kvällar"}`, detail: `${formatDuration(strategy.eveningWallSeconds, true)} föreslagen kvällstid` };
}

function plannerCapacitySummary(order) {
  const limited = (order?.moments || []).filter((moment) => moment.maxStaff || moment.resourceName);
  if (!limited.length) return "Ingen särskild momentgräns";
  const resourceMap = new Map();
  for (const moment of limited) {
    const name = String(moment.resourceName || "").trim();
    if (!name) continue;
    const capacity = Math.max(1, Math.floor(Number(moment.resourceCapacity) || 1));
    const key = name.toLocaleLowerCase("sv");
    const previous = resourceMap.get(key);
    if (!previous || capacity < previous.capacity) resourceMap.set(key, { name, capacity });
  }
  const resources = [...resourceMap.values()].map((resource) => `${resource.name} ×${resource.capacity}`);
  const labels = [];
  labels.push(`${limited.length} ${limited.length === 1 ? "begränsat moment" : "begränsade moment"}`);
  if (resources.length) labels.push(resources.join(", "));
  return labels.join(" · ");
}

function renderPlannedOrderList(schedule) {
  const byId = new Map(schedule.orders.map((item) => [item.id, item]));
  const search = $("#plannerSearch").value.trim().toLocaleLowerCase("sv");
  const resource = $("#plannerResourceFilter").value.toLocaleLowerCase("sv");
  const showComplete = $("#plannerShowComplete").checked;
  const rows = schedule.rows.filter((row) => (showComplete || !row.complete)
    && (!search || `${row.order.name} ${row.order.articleNumber} ${row.order.description}`.toLocaleLowerCase("sv").includes(search))
    && (!resource || row.order.moments.some((moment) => moment.resourceName.toLocaleLowerCase("sv") === resource)));
  $("#plannerFilterNote").textContent = `Visar ${rows.length} av ${schedule.rows.length} order på lina ${state.plannerConfig.line}. Visningsfilter ändrar inte linans bemanning eller tidsförslag.`;
  if (!rows.length) {
    dom.plannerOrderList.innerHTML = `<div class="planner-empty"><strong>${schedule.rows.length ? "Inga order matchar visningen" : "Lägg in arbetet ni har framför er"}</strong><span>${schedule.rows.length ? "Ändra sökning, arbetsplats eller visa klara order." : "Lägg nitorder och Rotary-order var för sig. Lägg sedan operationerna på respektive order och koppla de överlämningar som behövs."}</span></div>`;
    return;
  }
  dom.plannerOrderList.innerHTML = rows.map((row) => {
    const order = row.order;
    const result = byId.get(order.id);
    const labor = plannerLaborSeconds(order);
    const status = row.complete ? "Klar i planen" : row.ready ? "Körklar" : "Inväntar åtgärd";
    const dueLabel = order.dueKind === "delivery" ? "Leveransmål" : "Planerat färdig";
    const overdue = order.dueAt && new Date(order.dueAt) < new Date(state.plannerConfig.startAt) && !row.complete;
    const handoffs = order.dependencies.map((dependency) => {
      const source = state.plannedOrders.find((item) => item.id === dependency.orderId);
      return `<div class="order-handoff ${dependency.released ? "released" : "waiting"}"><span><b>${dependency.released ? "Överlämning klar" : "Inväntar"}</b> · ${escapeHtml(source?.name || "Saknad order")}</span><button class="text-button" type="button" data-handoff-order="${escapeHtml(order.id)}" data-handoff-source="${escapeHtml(dependency.orderId)}" ${!source ? "disabled" : ""}>${dependency.released ? "Markera väntar" : "Bekräfta överlämning"}</button></div>`;
    }).join("");
    return `<article class="planned-order-card ${row.ready ? "is-ready" : ""}" data-planned-order="${escapeHtml(order.id)}">
      <div class="planned-order-main">
        <div class="planned-order-title-row"><div><strong>${escapeHtml(order.name)}</strong><span>${escapeHtml(ORDER_TYPES[order.orderType])}${order.articleNumber ? ` · ${escapeHtml(order.articleNumber)}` : ""}${order.isExample ? " · Påhittat exempel" : ""}</span>${order.description ? `<span>${escapeHtml(order.description)}</span>` : ""}</div><span class="plan-status ${row.ready || row.complete ? "plan-ok" : "plan-wait"}">${status}</span></div>
        <p class="priority-reason">${escapeHtml(row.reason)}</p>
        ${row.blockedReason ? `<p class="planning-blocker">${escapeHtml(row.blockedReason)}</p>` : ""}
        ${order.needsReview ? '<p class="planning-notice">Äldre planering: kontrollera lina (förvald 4), ordertyp och körklar-status.</p>' : ""}
        <div class="planned-order-meta"><span class="${overdue ? "overdue-text" : ""}"><b>${dueLabel}</b> ${order.dueAt ? escapeHtml(formatTimestamp(order.dueAt)) : "Ej angivet"}${overdue ? " · Passerat vid planstart" : ""}</span>
          <span><b>Arbete kvar</b> ${labor === null ? "Tid saknas för minst en operation" : formatLaborTime(labor)}</span></div>
        <div class="operation-table-wrap"><table class="operation-table"><caption class="sr-only">Operationer på ${escapeHtml(order.name)}</caption><thead><tr><th>Operation / arbete</th><th>Arbetsplats</th><th>Kvar att göra</th><th>Persontid kvar</th></tr></thead><tbody>
          ${order.moments.map((moment, index) => { const time = momentLaborSeconds(order, moment); return `<tr class="${moment.remainingQuantity === 0 ? "operation-complete" : ""}"><th scope="row">${escapeHtml(operationLabel(moment, index))}</th><td>${escapeHtml(moment.resourceName || "Ej vald")}</td><td>${formatNumber(moment.remainingQuantity)} st</td><td>${time === null ? "Tid saknas" : formatLaborTime(time)}</td></tr>`; }).join("")}
        </tbody></table></div>
        ${handoffs}
        ${order.note ? `<p class="order-planning-note">${escapeHtml(order.note)}</p>` : ""}
        ${result?.finishAt ? `<div class="planned-order-forecast"><span>Uppskattad start <b>${escapeHtml(formatTimestamp(result.startAt))}</b></span><span>Uppskattat klar <b>${escapeHtml(formatTimestamp(result.finishAt))}</b></span><span class="${result.onTime === false ? "overdue-text" : ""}">${order.dueAt ? escapeHtml(plannerMarginText(result.marginMilliseconds)) : "Måldatum saknas"}</span></div>` : !row.complete ? `<p class="field-help">Tidsförslag saknas: ${escapeHtml(row.forecastBlocker || "ingen tillgänglig bemanning i planen")}. Prioriteten visas ändå.</p>` : ""}
        <p class="planning-updated">${order.updatedAt ? `Bedömning uppdaterad ${escapeHtml(formatTimestamp(order.updatedAt))}` : "Bedömningen har inte uppdaterats i denna version"}</p>
      </div>
      <div class="planned-order-actions"><button class="quiet-button mini-button" type="button" data-plan-edit="${escapeHtml(order.id)}">Uppdatera order</button>
        <button class="text-button" type="button" data-plan-open="${escapeHtml(order.id)}" ${state.order || order.moments.some((moment) => !moment.leadTime) || row.complete ? 'disabled title="Kalkylen behöver stycktider för alla operationer och en ledig orderkalkyl"' : ""}>Till orderkalkyl</button>
        <button class="danger-link" type="button" data-plan-delete="${escapeHtml(order.id)}">Ta bort</button></div>
    </article>`;
  }).join("");
}

function formatLaborTime(seconds) {
  const hours = Math.floor(seconds / 3600);
  const minutes = Math.round((seconds - hours * 3600) / 60);
  if (seconds > 0 && seconds < 60) return `${Math.round(seconds)} sek`;
  return `${hours + (minutes === 60 ? 1 : 0)} h${minutes % 60 ? ` ${minutes % 60} min` : ""}`;
}

function renderNextWork(schedule) {
  const names = [...new Set([...(state.plannerConfig.line === "4" ? WORKPLACES : []),
    ...schedule.rows.flatMap((row) => row.order.moments.map((moment) => moment.resourceName)).filter(Boolean)])]
    .filter((name, index, all) => all.findIndex((other) => other.toLocaleLowerCase("sv") === name.toLocaleLowerCase("sv")) === index);
  const selected = $("#plannerResourceFilter").value;
  $("#plannerResourceFilter").innerHTML = '<option value="">Alla arbetsplatser</option>' + names.map((name) => `<option value="${escapeHtml(name)}" ${name === selected ? "selected" : ""}>${escapeHtml(name)}</option>`).join("");
  const hasOpenOrders = schedule.rows.some((row) => !row.complete);
  if (!hasOpenOrders) {
    $("#plannerNextWork").innerHTML = `<div class="planner-empty"><strong>${schedule.rows.length ? "Alla order är klara i planen" : "Börja med en nitorder och en Rotary-order"}</strong><span>Ange arbetsplats, kvarvarande arbete och vad som behöver lämnas över.</span><button class="quiet-button" type="button" data-plan-example>Prova ett påhittat Rotary-exempel</button></div>`;
    return;
  }
  $("#plannerNextWork").innerHTML = names.map((name) => {
    const queue = schedule.rows.filter((row) => !row.complete && row.currentMoment?.resourceName.toLocaleLowerCase("sv") === name.toLocaleLowerCase("sv"));
    const row = queue.find((item) => item.ready);
    const known = schedule.rows.reduce((sum, item) => sum + item.order.moments.filter((moment) => moment.resourceName.toLocaleLowerCase("sv") === name.toLocaleLowerCase("sv")).reduce((total, moment) => total + (momentLaborSeconds(item.order, moment) ?? 0), 0), 0);
    return `<article class="next-work-card ${row ? "has-work" : ""}"><div class="next-work-resource"><strong>${escapeHtml(name)}</strong><span>${formatLaborTime(known)} känd belastning</span></div>
      ${row ? `<strong class="next-work-order">${escapeHtml(row.order.name)}</strong><p>${escapeHtml(operationLabel(row.currentMoment, row.order.moments.indexOf(row.currentMoment)))} · ${formatNumber(row.currentMoment.remainingQuantity)} st kvar</p><p class="priority-reason">${escapeHtml(row.reason)}</p><button class="text-button" type="button" data-plan-edit="${escapeHtml(row.order.id)}">Öppna order</button>` : '<strong class="next-work-order">Ingen körklar order</strong>'}
      <span class="next-work-footer">${queue.filter((item) => item.ready).length} körbara · ${queue.filter((item) => !item.ready).length} inväntar åtgärd</span></article>`;
  }).join("") || '<div class="planner-empty"><strong>Välj arbetsplats på orderns operationer</strong><span>Då kan nästa jobb visas här.</span></div>';
}

function renderPlannerSchedule(schedule) {
  const groups = plannerConcurrentGroups(schedule);
  if (!groups.length) {
    dom.plannerScheduleList.innerHTML = '<div class="planner-empty"><strong>Inget tidsförslag ännu</strong><span>Tidsförslaget behöver arbetsplatser, stycktider och tillgänglig bemanning. Kontrollera även körklar-status och kopplade order. Prioriteringen ovan kan användas under tiden.</span></div>';
    return;
  }

  const orderMap = new Map(state.plannedOrders.map((order) => [order.id, order]));
  const resourceMap = new Map((schedule.resources || []).map((resource) => [String(resource.key || resource.name || "").toLocaleLowerCase("sv"), resource]));

  dom.plannerScheduleList.innerHTML = groups.map((group, index) => {
    const shiftLabel = group.shift === "evening" ? "Kvällsskift" : "Dagskift";
    const staffCapacity = group.shift === "evening" ? state.plannerConfig.eveningStaff : state.plannerConfig.dayStaff;
    const totalStaff = group.items.reduce((sum, item) => sum + Math.max(0, Number(item.staff) || 0), 0);
    const simultaneousCount = group.items.length;

    const resourceUsage = new Map();
    for (const item of group.items) {
      const name = String(item.resourceName || "").trim();
      if (!name) continue;
      const key = name.toLocaleLowerCase("sv");
      const configured = resourceMap.get(key);
      const capacity = Math.max(1, Math.floor(Number(configured?.capacity ?? item.resourceCapacity) || 1));
      const current = resourceUsage.get(key) || { name: configured?.name || name, used: 0, capacity };
      current.used += Math.max(0, Number(item.staff) || 0);
      current.capacity = Math.min(current.capacity, capacity);
      resourceUsage.set(key, current);
    }

    const capacityBits = [`Bemanning ${totalStaff}/${staffCapacity}`];
    for (const resource of resourceUsage.values()) capacityBits.push(`${resource.name} ${resource.used}/${resource.capacity}`);

    const jobs = group.items.map((segment) => {
      const order = orderMap.get(segment.orderId);
      const moment = segment.momentName || "Produktion";
      const resourceCapacity = Math.max(0, Math.floor(Number(segment.resourceCapacity) || 0));
      const resource = segment.resourceName
        ? `<span>${escapeHtml(segment.resourceName)}${resourceCapacity ? ` · ${segment.staff}/${resourceCapacity} platser` : ""}</span>`
        : "";
      return `
        <div class="schedule-job">
          <div class="schedule-job-title"><strong>${escapeHtml(order?.name || "Order")}</strong><span>${escapeHtml(moment)}</span></div>
          <div class="schedule-job-meta"><span>${segment.staff} ${segment.staff === 1 ? "montör" : "montörer"}</span>${resource}</div>
        </div>`;
    }).join("");

    return `
      <article class="schedule-group ${group.shift === "evening" ? "is-evening" : ""}">
        <header class="schedule-group-header">
          <span class="schedule-index">${index + 1}</span>
          <div class="schedule-group-time">
            <strong>${escapeHtml(plannerDateLabel(group.start))}</strong>
            <span>${escapeHtml(plannerClock(group.start))}–${escapeHtml(plannerClock(group.end))}</span>
          </div>
          <div class="schedule-group-badges">
            <span class="schedule-shift-badge">${shiftLabel}</span>
            ${simultaneousCount > 1 ? `<span class="schedule-parallel-badge">${simultaneousCount} samtidigt</span>` : ""}
          </div>
          <div class="schedule-capacity-summary">${capacityBits.map((bit) => `<span>${escapeHtml(bit)}</span>`).join("")}</div>
        </header>
        <div class="schedule-group-jobs">${jobs}</div>
      </article>`;
  }).join("");
}

function renderPlannerStrategyBanner(schedule) {
  const banner = $("#plannerStrategyBanner");
  const open = schedule.rows.filter((row) => !row.complete);
  banner.hidden = !open.length;
  if (!open.length) return;
  const missing = open.filter((row) => row.forecastBlocker).length;
  const unknownDue = open.filter((row) => !row.order.dueAt).length;
  const late = schedule.orders.filter((order) => order.onTime === false).length;
  const unscheduled = schedule.orders.filter((order) => !order.finishAt).length;
  const strategy = schedule.strategy || {};
  const parts = [];
  if (late) parts.push(`${late} order når inte sitt måldatum i tidsförslaget.`);
  if (missing) parts.push(`${missing} order saknar tidsförslag på grund av uppgifter eller hinder som behöver lösas.`);
  if (unscheduled) parts.push(`${unscheduled} order kan inte läggas ut med vald bemanning.`);
  if (unknownDue) parts.push(`${unknownDue} order saknar måldatum.`);
  if (strategy.eveningNeeded) parts.push(`Förslaget använder ${formatDuration(strategy.eveningWallSeconds, true)} kvällstid, med angiven kvällsbemanning.`);
  if (!parts.length) parts.push("De kända tiderna ryms före angivna måldatum. Kontrollera att förutsättningarna fortfarande stämmer.");
  banner.className = `planner-strategy-banner ${late || missing || unscheduled || unknownDue ? "still-risk" : "no-evening"}`;
  banner.innerHTML = `<div><strong>${late || missing || unscheduled || unknownDue ? "Förutsättningar att följa upp" : "Tidsförslag för linan"}</strong><span>${escapeHtml(parts.join(" "))}</span></div>`;
}

function renderPlannerSummary(schedule) {
  const open = schedule.rows.filter((row) => !row.complete);
  const moments = open.flatMap((row) => row.order.moments.filter((moment) => moment.remainingQuantity > 0).map((moment) => ({ order: row.order, moment })));
  const known = moments.reduce((sum, { order, moment }) => sum + (momentLaborSeconds(order, moment) ?? 0), 0);
  const missing = moments.filter(({ order, moment }) => momentLaborSeconds(order, moment) === null).length;
  const forecastById = new Map(schedule.orders.map((order) => [order.id, order]));
  const actions = open.filter((row) => !row.ready || row.forecastBlocker || !row.order.dueAt || !forecastById.get(row.order.id)?.finishAt || forecastById.get(row.order.id)?.onTime === false);
  const waiting = open.filter((row) => !row.ready).length;
  const late = schedule.orders.filter((order) => order.onTime === false).length;
  const evening = plannerEveningSummary(schedule);
  $("#plannerOrderCount").textContent = String(open.length);
  $("#plannerOperationCount").textContent = `${moments.length} återstående operationer`;
  $("#plannerLaborTotal").textContent = formatLaborTime(known);
  $("#plannerLaborDetail").textContent = missing ? `${missing} operationer saknar tid` : "uppskattad persontid inkl. tillägg";
  $("#plannerRiskCount").textContent = String(actions.length);
  $("#plannerRiskDetail").textContent = open.length ? `${waiting} inväntar · ${late} tidsrisk` : "Inga öppna order";
  $("#plannerEveningStatus").textContent = evening.title;
  $("#plannerEveningDetail").textContent = evening.detail;
}

function renderPlanner() {
  renderPlannerPresetOptions($("#plannerPresetSelect").value);
  const schedule = buildPlannerRecommendation();
  state.latestPlan = schedule;
  renderPlannerSummary(schedule);
  renderNextWork(schedule);
  renderPlannedOrderList(schedule);
  renderPlannerStrategyBanner(schedule);
  renderPlannerSchedule(schedule);
  const oldStart = !isSameDay(new Date(state.plannerConfig.startAt), new Date()) && new Date(state.plannerConfig.startAt) < new Date();
  $("#plannerStaleNote").hidden = !oldStart;
  $("#plannerStaleNote").textContent = "Planstart ligger på en tidigare dag. Välj Planera från nu för ett nytt förslag från dagens läge.";
  $("#plannerRemoveExamples").hidden = !state.plannedOrders.some((order) => order.isExample);
  $("#plannerExampleButton").disabled = state.plannedOrders.some((order) => order.isExample);
}

function savePlannerOrder(event) {
  event.preventDefault();
  const draft = plannerOrderDraft();
  const error = validatePlanningOrder(draft, state.plannedOrders);
  if (error) return showError(dom.plannerOrderError, error);
  const existing = state.plannedOrders.find((order) => order.id === draft.id);
  if (existing && existing.updatedAt !== state.plannerEditingUpdatedAt) {
    return showError(dom.plannerOrderError, "Ordern har uppdaterats medan formuläret var öppet. Öppna Uppdatera order igen innan du sparar.");
  }
  const normalized = normalizePlanningOrder(draft);
  if (existing) Object.assign(existing, normalized);
  else state.plannedOrders.push(normalized);
  savePlannedOrders();
  resetPlannerOrderForm();
  $("#plannerEditor").open = false;
  renderPlanner();
  $("#plannedOrdersTitle").scrollIntoView({ behavior: "smooth", block: "start" });
  toast(existing ? "Planeringsuppgifterna är uppdaterade" : `Ordern är tillagd på lina ${normalized.line}`);
}

function editPlannedOrder(orderId) {
  const order = state.plannedOrders.find((item) => item.id === orderId);
  if (!order) return;
  state.plannerEditingId = order.id;
  state.plannerEditingUpdatedAt = order.updatedAt;
  const fields = { plannerOrderName: order.name, plannerArticle: order.articleNumber, plannerDescription: order.description,
    plannerOrderType: order.orderType, plannerOrderLine: order.line, plannerQuantity: order.quantity, plannerPerCarton: order.perCarton,
    plannerBuffer: String(order.bufferPercent).replace(".", ","), plannerDueKind: order.dueKind,
    plannerDeadline: order.dueAt ? toLocalInputValue(order.dueAt) : "", plannerReadiness: order.readiness,
    plannerPriority: order.priority, plannerNote: order.note };
  Object.entries(fields).forEach(([field, value]) => { $(`#${field}`).value = String(value ?? ""); });
  renderPlannerPresetOptions(order.presetId);
  renderPlannerMoments(order.moments);
  renderPlannerDependencies(order.dependencies);
  $("#plannerFormTitle").textContent = `Uppdatera ${order.name}`;
  $("#savePlannedOrderButton").textContent = "Spara ändringar";
  $("#cancelPlannerEditButton").hidden = false;
  showError(dom.plannerOrderError, "");
  $("#plannerEditor").open = true;
  dom.plannerOrderForm.scrollIntoView({ behavior: "smooth", block: "start" });
}

function deletePlannedOrder(orderId) {
  const order = state.plannedOrders.find((item) => item.id === orderId);
  if (!order) return;
  const dependents = state.plannedOrders.filter((other) => other.dependencies.some((dependency) => dependency.orderId === order.id));
  if (dependents.length) return toast(`Ordern behövs i kopplingar till ${dependents.map((item) => item.name).join(", ")}. Ändra kopplingarna först.`);
  if (state.order?.plannedOrderId === order.id) return toast("Stäng den kopplade orderkalkylen innan ordern tas bort.");
  if (!window.confirm(`Ta bort ${order.name} från din lokala plan?`)) return;
  state.plannedOrders = state.plannedOrders.filter((item) => item.id !== orderId);
  savePlannedOrders();
  if (state.plannerEditingId === orderId) resetPlannerOrderForm();
  renderPlanner();
  toast("Ordern är borttagen från planen");
}

function transferPlannedOrder(orderId) {
  const order = state.plannedOrders.find((item) => item.id === orderId);
  if (!order) return;
  if (state.order) return toast("Stäng den aktiva kalkylen innan du öppnar nästa.");
  if (order.moments.some((moment) => !moment.leadTime)) return toast("Ange stycktider för alla operationer innan du använder orderkalkylen.");
  state.pendingPlannedOrderId = order.id;
  state.pendingPlannedUpdatedAt = order.updatedAt;
  state.currentView = "order";
  $("#orderName").value = order.name;
  $("#totalQuantity").value = String(order.quantity);
  $("#perCarton").value = String(order.perCarton);
  $("#bufferPercent").value = String(order.bufferPercent).replace(".", ",");
  const plannedResult = buildPlannerRecommendation().orders.find((item) => item.id === order.id);
  $("#includeEveningShift").checked = Boolean(plannedResult?.segments?.some((segment) => segment.shift === "evening"));
  $("#startDateTime").value = toLocalInputValue();
  renderMomentEditors(order.moments.map((moment, index) => ({ ...moment, uiId: moment.id,
    name: operationLabel(moment, index),
    preCompletedCartons: Math.floor((order.quantity - moment.remainingQuantity) / order.perCarton),
    preCompletedLoose: (order.quantity - moment.remainingQuantity) % order.perCarton })));
  renderPresetOptions(order.presetId);
  render();
  updatePreview();
  window.scrollTo({ top: 0, behavior: "smooth" });
  toast("Kalkylen är förifylld. Ordern och kopplingarna finns kvar i planen.");
}

function syncCalculationToPlan() {
  const calculation = state.order;
  const planned = state.plannedOrders.find((order) => order.id === calculation?.plannedOrderId);
  if (!planned) return toast("Den kopplade ordern saknas i planen.");
  if (planned.updatedAt !== calculation.plannedUpdatedAt) return toast("Planeringsordern har ändrats sedan kalkylen öppnades. Uppdatera kvar att göra direkt i planen.");
  if (planned.quantity !== calculation.quantity || planned.moments.length !== calculation.moments.length
    || !planned.moments.every((moment) => calculation.moments.some((item) => item.plannedMomentId === moment.id))) {
    return toast("Kalkylens mängd eller operationer har ändrats. Uppdatera kvar att göra direkt i planen.");
  }
  planned.moments.forEach((moment) => {
    const calculated = calculation.moments.find((item) => item.plannedMomentId === moment.id);
    moment.remainingQuantity = Math.max(0, calculation.quantity - completedQuantityForMoment(calculation, calculated));
  });
  planned.updatedAt = new Date().toISOString();
  calculation.plannedUpdatedAt = planned.updatedAt;
  savePlannedOrders();
  saveOrder();
  toast("Kvar att göra är uppdaterat i planen. Bekräfta överlämningar separat.");
}

function updatePlannerConfig() {
  const fields = [$("#plannerDayStaff"), $("#plannerEveningStaff")];
  if (!$("#plannerStartAt").value || !Number.isFinite(new Date($("#plannerStartAt").value).getTime())
    || fields.some((field) => field.value === "" || !Number.isInteger(Number(field.value)) || Number(field.value) < 0 || Number(field.value) > 100)) {
    showError($("#plannerConfigError"), "Ange en planstart och 0–100 personer per skift. Det tidigare förslaget visas tills värdena är giltiga.");
    return;
  }
  showError($("#plannerConfigError"), "");
  const previousLine = state.plannerConfig.line;
  state.plannerConfig = plannerConfigFromInputs();
  if (!state.plannerEditingId && previousLine !== state.plannerConfig.line) $("#plannerOrderLine").value = state.plannerConfig.line;
  savePlannerConfig();
  renderPlanner();
}

function toggleHandoff(orderId, sourceId) {
  const order = state.plannedOrders.find((item) => item.id === orderId);
  const dependency = order?.dependencies.find((item) => item.orderId === sourceId);
  if (!dependency || !state.plannedOrders.some((item) => item.id === sourceId)) return;
  if (state.plannerEditingId === orderId) return toast("Ändra överlämningen i det öppna orderformuläret och spara där.");
  dependency.released = !dependency.released;
  order.updatedAt = new Date().toISOString();
  savePlannedOrders();
  renderPlanner();
  toast(dependency.released ? "Överlämning bekräftad för denna körning" : "Ordern inväntar överlämning igen");
}

function addRotaryExample() {
  if (state.plannedOrders.some((order) => order.isExample)) return toast("Exemplet finns redan i planen.");
  state.plannedOrders.push(...rotaryExample(state.plannerConfig.startAt, state.plannerConfig.line, id()));
  savePlannedOrders();
  renderPlanner();
  $("#nextWorkTitle").scrollIntoView({ behavior: "smooth", block: "start" });
  toast("Tre påhittade exempelorder är tillagda. Befintliga order finns kvar.");
}

function removeRotaryExamples() {
  const exampleIds = new Set(state.plannedOrders.filter((order) => order.isExample).map((order) => order.id));
  if (state.order && exampleIds.has(state.order.plannedOrderId)) return toast("Stäng exempelorderns kalkyl först.");
  if (state.plannedOrders.some((order) => !order.isExample && order.dependencies.some((dependency) => exampleIds.has(dependency.orderId)))) return toast("En egen order är kopplad till ett exempel. Ändra den kopplingen först.");
  state.plannedOrders = state.plannedOrders.filter((order) => !order.isExample);
  savePlannedOrders();
  if (exampleIds.has(state.plannerEditingId)) resetPlannerOrderForm();
  renderPlanner();
  toast("Exempelorder borttagna");
}

function renderBreakRows(breaks) {
  $("#breakList").innerHTML = breaks.map((item, index) => `
    <div class="break-row" data-break-row>
      <input type="time" value="${escapeHtml(item.start)}" aria-label="Rast ${index + 1} börjar" data-break-start required />
      <span>till</span>
      <input type="time" value="${escapeHtml(item.end)}" aria-label="Rast ${index + 1} slutar" data-break-end required />
      <button class="remove-break" type="button" aria-label="Ta bort rast ${index + 1}" data-remove-break="${index}">×</button>
    </div>`).join("");
}

function fillSettingsForm(settings) {
  $$("#weekdayPicker input").forEach((input) => {
    input.checked = settings.workdays.map(Number).includes(Number(input.value));
  });
  $("#workdayStart").value = settings.dayStart;
  $("#workdayEnd").value = settings.dayEnd;
  $("#fridayEnd").value = settings.fridayEnd || "14:00";
  $("#fridayNoLunch").checked = settings.fridayNoLunch !== false;
  $("#defaultBuffer").value = String(settings.bufferPercent).replace(".", ",");
  $("#defaultLeadUnit").value = settings.defaultLeadUnit;
  renderBreakRows(settings.breaks || []);
  showError(dom.settingsError, "");
}

function collectSettingsForm() {
  return {
    ...state.settings,
    workdays: $$("#weekdayPicker input:checked").map((input) => Number(input.value)),
    dayStart: $("#workdayStart").value,
    dayEnd: $("#workdayEnd").value,
    fridayEnd: $("#fridayEnd").value,
    fridayNoLunch: $("#fridayNoLunch").checked,
    breaks: $$("[data-break-row]").map((row) => ({
      start: row.querySelector("[data-break-start]").value,
      end: row.querySelector("[data-break-end]").value,
    })),
    bufferPercent: parseDecimal($("#defaultBuffer").value),
    defaultLeadUnit: $("#defaultLeadUnit").value,
  };
}

function saveSettingsFromForm() {
  const next = collectSettingsForm();
  const validity = settingsAreValid(next);
  if (!validity.valid) {
    showError(dom.settingsError, validity.message);
    return;
  }
  if (!(next.bufferPercent >= 0 && next.bufferPercent <= 500)) {
    showError(dom.settingsError, "Ange ett giltigt standardpåslag mellan 0 och 500 %.");
    return;
  }
  state.settings = next;
  saveSettings();
  if (!state.order) $("#bufferPercent").value = String(next.bufferPercent).replace(".", ",");
  dom.settingsDialog.close();
  render();
  toast("Inställningarna är sparade");
}

function exportData() {
  const payload = {
    app: "Industrilåsappen",
    dataVersion: DATA_VERSION,
    exportedAt: new Date().toISOString(),
    settings: state.settings,
    presets: state.presets,
    plannedOrders: state.plannedOrders,
    plannerConfig: state.plannerConfig,
    activeOrder: state.order,
  };
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `industrilasappen-${new Date().toISOString().slice(0, 10)}.json`;
  link.click();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
  toast("Säkerhetskopian är skapad");
}

async function importData(file) {
  try {
    const payload = JSON.parse(await file.text());
    if (payload.app !== "Industrilåsappen") throw new Error("Fel filformat");
    const importedSettings = { ...cloneDefaultSettings(), ...(payload.settings || {}) };
    const validity = settingsAreValid(importedSettings);
    if (!validity.valid) throw new Error("Ogiltiga inställningar");
    const importedOrders = Array.isArray(payload.plannedOrders) ? payload.plannedOrders.map(normalizePlannedOrder) : state.plannedOrders;
    if (importedOrders.some((order) => !order) || new Set(importedOrders.map((order) => order.id)).size !== importedOrders.length) throw new Error("Ogiltiga order i säkerhetskopian");
    for (const order of importedOrders) {
      const error = validatePlanningOrder(order, importedOrders);
      if (error) throw new Error(error);
    }
    const importedOrder = migrateOrder(payload.activeOrder);
    state.settings = importedSettings;
    state.order = importedOrder;
    if (Array.isArray(payload.presets)) {
      localStorage.setItem(PRESETS_KEY, JSON.stringify(payload.presets));
      state.presets = loadPresets();
    }
    if (Array.isArray(payload.plannedOrders)) {
      state.plannedOrders = importedOrders;
      savePlannedOrders();
    }
    if (payload.plannerConfig) {
      state.plannerConfig = normalizePlannerConfig(payload.plannerConfig);
      savePlannerConfig();
    }
    saveSettings();
    saveOrder();
    state.pendingPlannedOrderId = null;
    state.pendingPlannedUpdatedAt = null;
    resetPlannerOrderForm();
    syncPlannerConfigInputs();
    renderPresetOptions();
    dom.settingsDialog.close();
    render();
    toast("Säkerhetskopian är importerad");
  } catch (error) {
    showError(dom.settingsError, `Säkerhetskopian kunde inte importeras: ${error.message || "kontrollera filen"}.`);
  }
}

function setNewOrderDefaults() {
  state.pendingPlannedOrderId = null;
  state.pendingPlannedUpdatedAt = null;
  $("#startDateTime").value = toLocalInputValue();
  $("#bufferPercent").value = String(state.settings.bufferPercent).replace(".", ",");
  $("#includeEveningShift").checked = false;
  renderMomentEditors([{ name: "Montering", leadTime: 45, leadUnit: state.settings.defaultLeadUnit, preCompletedCartons: 0, preCompletedLoose: 0 }]);
  renderPresetOptions();
  updatePreview();
}

function addBreakRow() {
  const current = collectSettingsForm();
  current.breaks.push({ start: "12:00", end: "12:15" });
  renderBreakRows(current.breaks);
}

function removeBreakRow(index) {
  const current = collectSettingsForm();
  current.breaks.splice(index, 1);
  renderBreakRows(current.breaks);
}

function addMomentEditor() {
  const moments = collectMomentEditors();
  moments.push(createEditorMoment({ name: moments.length === 1 ? "Slutmontering" : `Moment ${moments.length + 1}`, leadTime: 45, leadUnit: state.settings.defaultLeadUnit }, moments.length));
  renderMomentEditors(moments);
  updatePreview();
}

function removeMomentEditor(uiId) {
  const moments = collectMomentEditors();
  if (moments.length <= 1) return;
  renderMomentEditors(moments.filter((moment) => moment.uiId !== uiId));
  updatePreview();
}

function attachEvents() {
  $("#plannerNavButton").addEventListener("click", () => {
    state.currentView = state.currentView === "planner" ? "order" : "planner";
    if (state.currentView === "planner") {
      syncPlannerConfigInputs();
    }
    render();
    window.scrollTo({ top: 0, behavior: "smooth" });
  });
  dom.plannerOrderForm.addEventListener("submit", savePlannerOrder);
  $("#plannerPresetSelect").addEventListener("change", () => {
    updatePlannerPresetSummary();
  });
  $("#cancelPlannerEditButton").addEventListener("click", () => {
    resetPlannerOrderForm();
    $("#plannerEditor").open = false;
    renderPlanner();
  });
  ["#plannerStartAt", "#plannerDayStaff", "#plannerEveningStaff", "#plannerLine"].forEach((selector) => {
    $(selector).addEventListener("change", updatePlannerConfig);
  });
  dom.plannerView.addEventListener("click", (event) => {
    const open = event.target.closest("[data-plan-open]");
    const edit = event.target.closest("[data-plan-edit]");
    const remove = event.target.closest("[data-plan-delete]");
    const handoff = event.target.closest("[data-handoff-order]");
    if (open) transferPlannedOrder(open.dataset.planOpen);
    else if (edit) editPlannedOrder(edit.dataset.planEdit);
    else if (remove) deletePlannedOrder(remove.dataset.planDelete);
    else if (handoff) toggleHandoff(handoff.dataset.handoffOrder, handoff.dataset.handoffSource);
    else if (event.target.closest("[data-plan-example]")) addRotaryExample();
  });
  $("#plannerAddButton").addEventListener("click", () => {
    resetPlannerOrderForm();
    $("#plannerEditor").open = true;
    $("#plannerOrderName").focus({ preventScroll: true });
    dom.plannerOrderForm.scrollIntoView({ behavior: "smooth", block: "start" });
  });
  $("#plannerNowButton").addEventListener("click", () => {
    $("#plannerStartAt").value = toLocalInputValue();
    updatePlannerConfig();
  });
  $("#plannerApplyPreset").addEventListener("click", applyPlannerPreset);
  $("#plannerAddMoment").addEventListener("click", () => {
    renderPlannerMoments([...collectPlannerMoments(), {}]);
  });
  $("#plannerMomentList").addEventListener("click", (event) => {
    const button = event.target.closest("[data-plan-remove-moment], [data-plan-move]");
    if (!button) return;
    const moments = collectPlannerMoments();
    const index = moments.findIndex((moment) => moment.id === button.closest("[data-planner-moment]").dataset.plannerMoment);
    if (button.hasAttribute("data-plan-remove-moment") && moments.length > 1) moments.splice(index, 1);
    else if (button.hasAttribute("data-plan-move")) {
      const destination = index + Number(button.dataset.planMove);
      if (destination >= 0 && destination < moments.length) [moments[index], moments[destination]] = [moments[destination], moments[index]];
    }
    renderPlannerMoments(moments);
  });
  $("#plannerAddDependency").addEventListener("click", () => {
    renderPlannerDependencies([...collectPlannerDependencies(), { orderId: "", released: false }]);
  });
  $("#plannerDependencyList").addEventListener("click", (event) => {
    const button = event.target.closest("[data-plan-remove-dependency]");
    if (button) button.closest("[data-planning-dependency]").remove();
  });
  ["#plannerSearch", "#plannerResourceFilter", "#plannerShowComplete"].forEach((selector) => {
    $(selector).addEventListener("input", () => {
      if (state.latestPlan) renderPlannedOrderList(state.latestPlan);
    });
  });
  $("#plannerExampleButton").addEventListener("click", addRotaryExample);
  $("#plannerRemoveExamples").addEventListener("click", removeRotaryExamples);
  $("#syncPlanButton").addEventListener("click", syncCalculationToPlan);

  dom.orderForm.addEventListener("submit", startOrder);
  ["#totalQuantity", "#perCarton", "#includeEveningShift", "#bufferPercent", "#startDateTime"].forEach((selector) => {
    $(selector).addEventListener("input", updatePreview);
    $(selector).addEventListener("change", updatePreview);
  });
  dom.momentEditorList.addEventListener("input", updatePreview);
  dom.momentEditorList.addEventListener("change", updatePreview);
  dom.momentEditorList.addEventListener("click", (event) => {
    const button = event.target.closest("[data-remove-moment]");
    if (button) removeMomentEditor(button.closest("[data-moment-editor]").dataset.momentEditor);
  });
  $("#addMomentButton").addEventListener("click", addMomentEditor);

  $("#presetSelect").addEventListener("change", () => renderPresetOptions($("#presetSelect").value));
  $("#savePresetButton").addEventListener("click", saveCurrentAsPreset);
  $("#loadPresetButton").addEventListener("click", loadSelectedPreset);
  $("#deletePresetButton").addEventListener("click", deleteSelectedPreset);

  $("#settingsButton").addEventListener("click", () => {
    fillSettingsForm(state.settings);
    dom.settingsDialog.showModal();
  });
  $("#saveSettingsButton").addEventListener("click", saveSettingsFromForm);
  $("#resetSettingsButton").addEventListener("click", () => {
    fillSettingsForm(cloneDefaultSettings());
    toast("Standardvärdena är ifyllda – tryck Spara");
  });
  $("#addBreakButton").addEventListener("click", addBreakRow);
  $("#breakList").addEventListener("click", (event) => {
    const button = event.target.closest("[data-remove-break]");
    if (button) removeBreakRow(Number(button.dataset.removeBreak));
  });
  $("#exportButton").addEventListener("click", exportData);
  $("#importButton").addEventListener("click", () => $("#importFile").click());
  $("#importFile").addEventListener("change", (event) => {
    const [file] = event.target.files;
    if (file) importData(file);
    event.target.value = "";
  });

  $("#completeCartonButton").addEventListener("click", completeCarton);
  $("#undoButton").addEventListener("click", undoLastRegistration);
  $("#pauseButton").addEventListener("click", togglePause);
  $("#finishOrderButton").addEventListener("click", finishOrder);
  $("#orderMenuButton").addEventListener("click", () => dom.orderMenuDialog.showModal());
  dom.momentOverviewList.addEventListener("click", (event) => {
    const button = event.target.closest("[data-activate-moment]");
    if (button) switchActiveMoment(button.dataset.activateMoment);
  });

  $("#addLooseButton").addEventListener("click", () => {
    const moment = activeMoment(state.order);
    showError(dom.looseError, "");
    $("#looseQuantity").value = "";
    $("#looseQuantity").max = String(moment ? state.order.quantity - completedQuantityForMoment(state.order, moment) : 0);
    dom.looseDialog.showModal();
    setTimeout(() => $("#looseQuantity").focus(), 50);
  });
  $("#saveLooseButton").addEventListener("click", addLoosePieces);
  $("#looseForm").addEventListener("submit", (event) => {
    if (event.submitter?.value !== "cancel") {
      event.preventDefault();
      addLoosePieces();
    }
  });

  $("#addMemberButton").addEventListener("click", () => {
    showError(dom.memberError, "");
    $("#memberName").value = "";
    $("#memberStart").value = toLocalInputValue();
    dom.memberDialog.showModal();
    setTimeout(() => $("#memberName").focus(), 50);
  });
  $("#saveMemberButton").addEventListener("click", addMember);
  $("#memberForm").addEventListener("submit", (event) => {
    if (event.submitter?.value !== "cancel") {
      event.preventDefault();
      addMember();
    }
  });
  $("#teamList").addEventListener("click", (event) => {
    const button = event.target.closest("[data-stop-member]");
    if (button) stopMember(button.dataset.stopMember);
  });

  document.addEventListener("visibilitychange", () => {
    if (!document.hidden && state.order) renderActiveOrder();
  });
}

function initialize() {
  attachEvents();
  setNewOrderDefaults();
  resetPlannerOrderForm();
  syncPlannerConfigInputs();
  render();
  setInterval(() => {
    if (state.order && !document.hidden) renderActiveOrder();
  }, 30_000);

  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => navigator.serviceWorker.register("./sw.js").catch(() => {}));
  }
}

initialize();
 
