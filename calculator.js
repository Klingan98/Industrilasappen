...
export function recommendProductionSchedule({
  ...
  const work = (orders || []).map((order) => {
    ...
    const priorityDueDate = order.priorityDueAt ? new Date(order.priorityDueAt) : dueDate;
  ...
  }).filter((order) => !order.dueDate || Number.isFinite(order.dueDate.getTime()));

  const workById = new Map(work.map((order) => [order.id, order]));
  const dependenciesComplete = (order) => (order.dependsOn || []).every((id) => {
    const source = workById.get(id);
    return source && source.remainingLaborSeconds <= 1e-6;
  });
  ...
  // Ungefärlig minsta klocktid som återstår om varje moment får använda sin
  // fulla tillåtna bemanning och resurskapacitet. Detta fångar flaskhalsar som
  // exempelvis en ensam press eller två tillgängliga fixturer.
  ...
  const availableWallSecondsToDue = (order, instant) => !order.priorityDueDate ? Infinity : scheduledWorkingSecondsBetween(
    instant,
    order.priorityDueDate,
  ...
  const rankedReadyOrders = (instant, shift) => work
    .filter((order) => {
      if (!(order.remainingLaborSeconds > 1e-6) || !currentTask(order) || !dependenciesComplete(order)) return false;
      return shift !== "evening" || Boolean(order.includeEveningShift);
    })
    .sort((a, b) => {
      if (Number.isFinite(a.dispatchRank) && Number.isFinite(b.dispatchRank) && a.dispatchRank !== b.dispatchRank) return a.dispatchRank - b.dispatchRank;
      ...
      const aDue = a.priorityDueDate?.getTime() ?? Infinity;
      const bDue = b.priorityDueDate?.getTime() ?? Infinity;
  ...
  const allocateAt = (instant, interval) => {
    ...
    // Ge först så många körbara order som möjligt en montör var. Då används
    // exempelvis två identiska pressar parallellt när det finns två montörer.
    for (const order of ready) {
      if (!staffLeft) break;
      assignOne(order);
    }

    // Lägg därefter extra montörer där nästa person ger störst tidsvinst, med
    // högre vikt för order som har liten eller negativ marginal till deadline.
    while (staffLeft > 0) {
      let bestOrder = null;
      ...
      for (const order of ready) {
        ...
        const score = marginalWallGain * pressureWeight * latenessWeight;
    ...
    return ready
      .map((order) => {
        ...
        return staff > 0 ? { order, task: currentTask(order), staff } : null;
      })
      .filter(Boolean);
  };
  ...
  for (let guard = 0; guard < 3700 && work.some((order) => order.remainingLaborSeconds > 1e-6); guard += 1) {
    // Missing/circular dependencies or no enabled staffed shift cannot resolve
    // just by advancing the calendar. Keep these orders visibly unscheduled.
    if (!work.some((order) => order.remainingLaborSeconds > 1e-6 && dependenciesComplete(order)
      && (Number(dayStaff) > 0 || (order.includeEveningShift && Number(eveningStaff) > 0)))) break;
  ...
  const results = work.map((order) => ({
    ...
    marginMilliseconds: order.finishAt && order.dueDate ? order.dueDate.getTime() - order.finishAt.getTime() : null,
    onTime: order.dueDate ? Boolean(order.finishAt && order.finishAt <= order.dueDate) : null,
  }));
...
function scheduleScore(schedule) {
  const lateOrders = (schedule?.orders || []).filter((order) => order.onTime === false);
  ...
  return {
    unscheduledCount: (schedule?.orders || []).filter((order) => !order.finishAt).length,
...
function scoreIsBetter(candidate, current) {
  ...
  if (a.unscheduledCount !== b.unscheduledCount) return a.unscheduledCount < b.unscheduledCount;
  // Optimera hela kön: först minsta sammanlagda deadlineförsening, sedan
  // minsta värsta försening och därefter antalet sena ordrar.
...
export function recommendAutomaticEveningSchedule({
  ...
  if (baseline.orders.some((order) => order.onTime === false || !order.finishAt) && normalizedOrders.length && Number(eveningStaff) > 0) {
