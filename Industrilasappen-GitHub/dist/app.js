...
function completedQuantity(order = state.order) {
  return Math.min(order?.quantity || 0, (order?.events || []).reduce((sum, event) => sum + (Number(event.quantity) || 0), 0));
}
...
function forecastForOrder(order) {
  ...
  const measuredPieceSeconds = done > 0 ? workedPersonSeconds / done : 0;
  const hasMeasuredPace = done > 0 && workedPersonSeconds >= 30;
...
function updatePreview() {
  ...
  if (!(quantity > 0 && perCarton > 0 && leadTime > 0 && bufferPercent >= 0 && Number.isFinite(start.getTime()))) {
  ...
  const finish = addWorkingSeconds(start, plan.plannedTotalSeconds, state.settings);
  $("#previewFinish").textContent = formatFinish(finish);
  $("#previewCartons").textContent = String(Math.ceil(quantity / perCarton));
  $("#previewBaseTime").textContent = formatDuration(plan.originalTotalSeconds, true);
  $("#previewBufferedTime").textContent = formatDuration(plan.plannedTotalSeconds, true);
...
function renderActiveOrder() {
  ...
  if (!forecast.hasMeasuredPace) {
    ...
    $("#teamPace").textContent = "Inväntar första kartongen";
...
function renderTeamList(now) {
  ...
  $("#teamList").innerHTML = order.members.map((member) => {
    ...
    const worked = productiveSecondsBetween(member.joinedAt, member.leftAt || now, state.settings, order.pauseEvents || []);
    return `
...
function attachEvents() {
  ...
  ["#totalQuantity", "#perCarton", "#leadTime", "#leadUnit", "#bufferPercent", "#startDateTime"].forEach((selector) => {
